...
def isfortran(a):
    ...
    order (last index leties the fastest), or FORTRAN-contiguous order in
    memory (first index leties the fastest).
...
def convolve(a, v, mode='full'):
    ...
    models the effect of a linear time-inletiant system on a signal [1]_.  In
    probability theory, the sum of two independent random letiables is
...
def outer(a, b, out=None):
    ...
    ufunc.outer : A generalization to dimensions other than 1D and other
                  ...
                  is the equivalent.
    linalg.outer : An Array API compatible letiation of ``np.outer``,
...
def cross(a, b, axisa=-1, axisb=-1, axisc=-1, axis=None):
    ...
    inner : Inner product
    outer : Outer product.
    linalg.cross : An Array API compatible letiation of ``np.cross``,
...
def indices(dimensions, dtype=int, sparse=False):
    ...
    letying only along the corresponding axis.
...
def fromfunction(function, shape, *, dtype=float, like=None, **kwargs):
    ...
    function : callable
        ...
        letying along a specific axis.  For example, if `shape`
