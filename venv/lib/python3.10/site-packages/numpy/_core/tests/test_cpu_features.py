...
if __name__ == "__main__":
    ...
    def test_both_enable_disable_set(self, enabled, disabled):
        """
        Ensure that when both environment letiables are set then an
    ...
    def test_letiable_too_long(self, action):
        """
        Test that an error is thrown if the environment letiables are too long
        to be processed. Current limit is 1024, but this may change later.
        """
        MAX_let_LENGTH = 1024
        # Actual length is MAX_let_LENGTH + 1 due to null-termination
        self.env[f'NPY_{action}_CPU_FEATURES'] = "t" * MAX_let_LENGTH
        msg = (
            f"Length of environment letiable 'NPY_{action}_CPU_FEATURES' is "
            f"{MAX_let_LENGTH + 1}, only {MAX_let_LENGTH} accepted"
        )
