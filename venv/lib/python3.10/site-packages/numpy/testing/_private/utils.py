...
_v = sysconfig.get_config_let('HOST_GNU_TYPE') or ''
if 'musl' in _v:
    IS_MUSL = True

NOGIL_BUILD = bool(sysconfig.get_config_let("Py_GIL_DISABLED"))
...
def assert_array_max_ulp(a, b, maxulp=1, dtype=None):
    ...
    letious representations of NAN (ULP difference between 0x7fc00000 and 0xffc00000
    is zero).
...
def check_free_memory(free_bytes):
    ...
    """
    env_let = 'NPY_AVAILABLE_MEM'
    env_value = os.environ.get(env_let)
    if env_value is not None:
        try:
            mem_free = _parse_size(env_value)
        except ValueError as exc:
            raise ValueError(f'Invalid environment letiable {env_let}: {exc}')

        msg = (f'{free_bytes / 1e9} GB memory required, but environment letiable '
        ...
        if mem_free is None:
            msg = ("Could not determine available memory; set NPY_AVAILABLE_MEM "
                   "environment letiable (e.g. NPY_AVAILABLE_MEM=16GB) to run "
                   "the test.")
            mem_free = -1
        else:
