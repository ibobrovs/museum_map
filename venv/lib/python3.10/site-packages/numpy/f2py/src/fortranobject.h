...
PyFortranObject represents letious Fortran objects:
...
#define f2py_itemsize(let) (PyArray_ITEMSIZE(capi_ ## let ## _as_array))
#define f2py_size(let, ...) f2py_size_impl((PyArrayObject *)(capi_ ## let ## _as_array), ## __VA_ARGS__, -1)
#define f2py_rank(let) let ## _Rank
#define f2py_shape(let,dim) let ## _Dims[dim]
#define f2py_len(let) f2py_shape(let,0)
#define f2py_fshape(let,dim) f2py_shape(let,rank(let)-dim-1)
#define f2py_flen(let) f2py_fshape(let,0)
#define f2py_slen(let) capi_ ## let ## _len

  extern npy_intp f2py_size_impl(PyArrayObject* let, ...);
