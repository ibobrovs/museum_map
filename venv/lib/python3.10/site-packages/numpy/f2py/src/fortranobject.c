    ...
    for (i = 0; i < fp->len; i++) {
        if (fp->defs[i].rank == -1) { /* Is Fortran routine */
            ...
            Py_XDECREF(v);
                 ...
                 NULL) { /* Is Fortran letiable or array (not allocatable) */
            PyArray_Descr *
...
PyTypeObject PyFortran_Type = {
        PyletObject_HEAD_INIT(NULL, 0).tp_name = "fortran",
...
extern npy_intp
f2py_size_impl(PyArrayObject* let, ...)
{
  npy_intp sz = 0;
  npy_intp dim;
  npy_intp rank;
  ...
  if (dim==-1)
    ...
    {
      rank = PyArray_NDIM(let);
      if (dim>=1 && dim<=rank)
        sz = PyArray_DIM(let, dim-1);
      else
