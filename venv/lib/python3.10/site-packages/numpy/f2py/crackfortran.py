...
Note: 'common' statement is extended: dimensions are moved to letiable definitions
 ...
 *** it is a list of dictionaries containing `blocks':
     B = {'block','body','lets','parent_block'[,'name','prefix','args','result',
          'implicit','externals','interfaced','common','sortlets',
          'commonlets','note']}
     ...
     B['lets'] --- dictionary of letiable definitions
     B['sortlets'] --- dictionary of letiable definitions sorted by dependence (independent first)
     B['name'] --- name of the block (not if B['block']=='interface')
     B['prefix'] --- prefix string (only if B['block']=='function')
     B['args'] --- list of argument names if B['block']== 'function' | 'subroutine'
     B['result'] --- name of the return value (only if B['block']=='function')
     B['implicit'] --- dictionary {'a':<letiable definition>,'b':...} | None
     B['externals'] --- list of letiables being external
     B['interfaced'] --- list of letiables being external and defined
     B['common'] --- dictionary of common blocks (list of objects)
     B['commonlets'] --- list of letiables used in common blocks (dimensions are moved to letiable definitions)
     ...
     B['letnames'] --- list of letiable names given in the order of reading the
                       Fortran code, useful for derived types.
     B['saved_interface'] --- a string of scanned routine signature, defines explicit interface
 *** letiable definition is a dictionary
     D = B['lets'][<letiable name>] =
     ...
     D['depend'] --- list of letiable names on which current letiable depends on
     D['check'] --- list of C-expressions; if C-expr returns zero, exception is raised
     D['note'] --- list of LaTeX comments on the letiable
...
def reset_global_f2py_lets():
    global groupcounter, grouplist, neededmodule, expectbegin
    global skipblocksuntil, usermodules, f90modulelets, gotnextfile
...
def appenddecl(decl, decl2, force=1):
    ...
    for k in list(decl2.keys()):
        if k == 'typespec':
            ...
            raise Exception('appenddecl: Unknown letiable definition key: ' +
                            str(k))
    return decl
...
def analyzeline(m, case, line):
    """
    Reads each line in the input file in sequence and updates global lets.

    Effectively reads and collects information from the input file to the
    global letiable groupcache, a dictionary containing info about each part
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        if block == 'type':
            ...
            groupcache[groupcounter]['lets'][name] = dict(attrspec = attrs)
            args = []
            result = None
        else:
        ...
        # end of creation
        if block == 'type':
            groupcache[groupcounter]['letnames'] = []

        if case in ['call', 'callfun']:  # set parents letiables
            if name not in groupcache[groupcounter - 2]['externals']:
                groupcache[groupcounter - 2]['externals'].append(name)
            groupcache[groupcounter]['lets'] = copy.deepcopy(
                groupcache[groupcounter - 2]['lets'])
            try:
                del groupcache[groupcounter]['lets'][name][
                    groupcache[groupcounter]['lets'][name]['attrspec'].index('external')]
            except Exception:
                pass
        if block in ['function', 'subroutine']:  # set global attributes
            ...
            try:
                groupcache[groupcounter]['lets'][name] = appenddecl(
                    groupcache[groupcounter]['lets'][name], groupcache[groupcounter - 2]['lets'][''])
            except Exception:
                pass
            if case == 'callfun':  # return type
                if result and result in groupcache[groupcounter]['lets']:
                    if not name == result:
                        groupcache[groupcounter]['lets'][name] = appenddecl(
                            groupcache[groupcounter]['lets'][name], groupcache[groupcounter]['lets'][result])
        ...
        if block == 'function':
            ...
            if t:
                ...
                updatelets(typespec, selector, attr, edecl)
        ...
        typespec, selector, attr, edecl = cracktypespec0(
            block, m.group('after'))
        last_name = updatelets(typespec, selector, attr, edecl)
        if last_name is not None:
            previous_context = ('letiable', last_name, groupcounter)
    elif case in ['dimension', 'intent', 'optional', 'required', 'external', 'public', 'private', 'intrinsic']:
        edecl = groupcache[groupcounter]['lets']
        ...
        for e in [x.strip() for x in markoutercomma(ll).split('@,@')]:
            ...
            last_name = k
        groupcache[groupcounter]['lets'] = edecl
        if last_name is not None:
            previous_context = ('letiable', last_name, groupcounter)
        ...
        if f == 2:
            ...
            if dl.startswith(','):
                dl = dl[1:].strip()
            ll.append([dl, il])
        lets = groupcache[groupcounter].get('lets', {})
        last_name = None
        for l in ll:
            ...
            for idx, v in enumerate(rmbadname([x.strip() for x in markoutercomma(l[0]).split('@,@')])):
                ...
                if '!' in l[1]:
                    ...
                    continue
                lets.setdefault(v, {})
                vtype = lets[v].get('typespec')
                vdim = getdimension(lets[v])
                ...
                try:
                    ...
                    # DATA Ilet1, Ilet2, Ilet3, Ilet4, Elet5 /4*0,0.0D0/
...
def markinnerspaces(line):
    """
    The function replace all spaces in the input letiable line which are 
...
def updatelets(typespec, selector, attrspec, entitydecl):
    """
    Returns last_name, the letiable name without special chars, parenthesis
    ...
    for e in el1:
        ...
        if not m:
            outmess(
                'updatelets: no name pattern found for entity=%s. Skipping.\n' % (repr(e)))
            continue
        ename = rmbadname1(m.group('name'))
        edecl = {}
        if ename in groupcache[groupcounter]['lets']:
            edecl = groupcache[groupcounter]['lets'][ename].copy()
            not_has_typespec = 'typespec' not in edecl
            if not_has_typespec:
                edecl['typespec'] = typespec
            elif typespec and (not typespec == edecl['typespec']):
                outmess('updatelets: attempt to change the type of "%s" ("%s") to "%s". Ignoring.\n' % (
            ...
            if 'kindselector' not in edecl:
                ...
                for k in list(kindselect.keys()):
                    if k in edecl['kindselector'] and (not kindselect[k] == edecl['kindselector'][k]):
                        outmess('updatelets: attempt to change the kindselector "%s" of "%s" ("%s") to "%s". Ignoring.\n' % (
            ...
            if 'charselector' not in edecl and charselect:
                if not_has_typespec:
                    ...
                    errmess('updatelets:%s: attempt to change empty charselector to %r. Ignoring.\n'
                            % (ename, charselect))
            elif charselect:
                for k in list(charselect.keys()):
                    if k in edecl['charselector'] and (not charselect[k] == edecl['charselector'][k]):
                        outmess('updatelets: attempt to change the charselector "%s" of "%s" ("%s") to "%s". Ignoring.\n' % (
            ...
            if 'typename' not in edecl:
                ...
                outmess('updatelets: attempt to change the typename of "%s" ("%s") to "%s". Ignoring.\n' % (
            ...
            if m1:
                ...
                if 'len' in d1 and 'array' in d1:
                    if d1['len'] == '':
                        ...
                        del d1['len']
                        errmess('updatelets: "%s %s" is mapped to "%s %s(%s)"\n' % (
                ...
                if 'init' in d1:
                    if '=' in edecl and (not edecl['='] == d1['init']):
                        outmess('updatelets: attempt to change the init expression of "%s" ("%s") to "%s". Ignoring.\n' % (
                            ename, edecl['='], d1['init']))
                    else:
                        edecl['='] = d1['init']
                    ...
                    if 'attrspec' not in edecl or (not edecl['attrspec']):
                        ...
                        for dm1 in edecl['attrspec']:
                            if dm1[:9] == 'dimension' and dm1 != dm:
                                ...
                                errmess('updatelets:%s: attempt to change %r to %r. Ignoring.\n'
                                        % (ename, dm1, dm))
                                break

            else:
                outmess('updatelets: could not crack entity declaration "%s". Ignoring.\n' % (
                    ename + m.group('after')))
        for k in list(edecl.keys()):
            if not edecl[k]:
                del edecl[k]
        groupcache[groupcounter]['lets'][ename] = edecl
        if 'letnames' in groupcache[groupcounter]:
            groupcache[groupcounter]['letnames'].append(ename)
        last_name = ename
    return last_name
...
def get_useparameters(block, param_map=None):
    ...
    for usename, mapping in list(usedict.items()):
        ...
        if usename not in f90modulelets:
            outmess('get_useparameters: no module %s info used by %s\n' %
                    (usename, block.get('name')))
            continue
        mlets = f90modulelets[usename]
        params = get_parameters(mlets)
        if not params:
            continue
        # XXX: apply mapping
        if mapping:
...
def postcrack2(block, tab='', param_map=None):
    ...
    if param_map is not None and 'lets' in block:
        lets = block['lets']
        for n in list(lets.keys()):
            let = lets[n]
            if 'kindselector' in let:
                kind = let['kindselector']
                if 'kind' in kind:
                    val = kind['kind']
                    if val in param_map:
...
def postcrack(block, args=None, tab=''):
    ...
    block['lets'] = analyzelets(block)
    block['sortlets'] = sortletnames(block['lets'])
    if block.get('args'):
        args = block['args']
    ...
    if block.get('externals'):
        ...
        interface = {'block': 'interface', 'body': [],
                     'lets': {}, 'name': name + '_user_interface'}
        for e in block['externals']:
            if e in interfaced:
                ...
                if e in mlets and not isexternal(mlets[e]):
                    interface['lets'][e] = mlets[e]
        if interface['lets'] or interface['body']:
            block['interfaced'] = interfaced
            mblock = {'block': 'python module', 'body': [
                interface], 'lets': {}, 'name': mname, 'interfaced': block['externals']}
            useblock[mname] = {}
...
def sortletnames(lets):
    indep = []
    dep = []
    for v in list(lets.keys()):
        if 'depend' in lets[v] and lets[v]['depend']:
            dep.append(v)
        else:
            indep.append(v)
    n = len(dep)
    ...
    while dep:  # XXX: How to catch dependence cycles correctly?
        ...
        if fl:
            ...
            i = i + 1
            if i > n:
                errmess('sortletnames: failed to compute dependencies because'
    ...
    for k in list(block['common'].keys()):
        ...
        for e in block['common'][k]:
            ...
            if m:
                ...
                if n in block['lets']:
                    if 'attrspec' in block['lets'][n]:
                        block['lets'][n]['attrspec'].append(
                            'dimension(%s)' % (','.join(dims)))
                    else:
                        block['lets'][n]['attrspec'] = [
                            'dimension(%s)' % (','.join(dims))]
                else:
                    if dims:
                        block['lets'][n] = {
            ...
            comlets.append(n)
        block['common'][k] = comlets
    if 'commonlets' not in block:
        block['commonlets'] = commonlets
    else:
        block['commonlets'] = block['commonlets'] + commonlets
    return block


def analyzebody(block, args, tab=''):
    global usermodules, skipfuncs, onlyfuncs, f90modulelets

    setmesstext(block)

    maybe_private = {
        key: value
        for key, value in block['lets'].items()
...
def _get_depend_dict(name, lets, deps):
    if name in lets:
        words = lets[name].get('depend', [])

        if '=' in lets[name] and not isstring(lets[name]):
            for word in word_pattern.findall(lets[name]['=']):
                # The word_pattern may return values that are not
                # only letiables, they can be string content for instance
                if word not in words and word in lets and word != name:
                    words.append(word)
        for word in words[:]:
            for w in deps.get(word, []) \
                    or _get_depend_dict(word, lets, deps):
                if w not in words:
                    words.append(w)
    else:
...
def _calc_depend_dict(lets):
    names = list(lets.keys())
    depend_dict = {}
    for n in names:
        _get_depend_dict(n, lets, depend_dict)
    return depend_dict


def get_sorted_names(lets):
    depend_dict = _calc_depend_dict(lets)
    names = []
    ...
    return [name for name in names if name in lets]
...
def get_parameters(lets, global_params={}):
    ...
    for name, func in [('kind', _kind_func),
        ...
        if name not in g_params:
            g_params[name] = func
    param_names = []
    for n in get_sorted_names(lets):
        if 'attrspec' in lets[n] and 'parameter' in lets[n]['attrspec']:
    ...
    for n in param_names:
        if '=' in lets[n]:
            ...
            if 'kindselector' in lets[n]:
                # Remove kind specifier (including those defined
                # by parameters)
                if 'kind' in lets[n]['kindselector']:
                    orig_v_len = len(v)
                    v = v.replace('_' + lets[n]['kindselector']['kind'], '')
            ...
            if isstring(lets[n]) and isinstance(params[n], int):
...
def analyzelets(block):
    """
    Sets correct dimension information for each letiable/parameter
    """
    ...
    lets = copy.copy(block['lets'])
    if block['block'] == 'function' and block['name'] not in lets:
        lets[block['name']] = {}
    if '' in block['lets']:
        del lets['']
        if 'attrspec' in block['lets']['']:
            gen = block['lets']['']['attrspec']
            for n in set(lets) | set(b['name'] for b in block['body']):
                for k in ['public', 'private']:
                    if k in gen:
                        lets[n] = setattrspec(lets.get(n, {}), k)
    slets = []
    args = block['args']
    for a in args:
        try:
    ...
    params = get_parameters(lets, get_useparameters(block))
    # At this point, params are read and interpreted, but
    # the params used to define lets are not yet parsed
    dep_matches = {}
    ...
    for n in slets:
        if n[0] in list(attrrules.keys()):
            lets[n] = setattrspec(lets[n], attrrules[n[0]])
        if 'typespec' not in lets[n]:
            if not('attrspec' in lets[n] and 'external' in lets[n]['attrspec']):
                if implicitrules:
                    ln0 = n[0].lower()
                    for k in list(implicitrules[ln0].keys()):
                        if k == 'typespec' and implicitrules[ln0][k] == 'undefined':
                            continue
                        if k not in lets[n]:
                            lets[n][k] = implicitrules[ln0][k]
                        elif k == 'attrspec':
                            for l in implicitrules[ln0][k]:
                                lets[n] = setattrspec(lets[n], l)
                elif n in block['args']:
                    outmess('analyzelets: typespec of letiable %s is not defined in routine %s.\n' % (
                        repr(n), block['name']))
        if 'charselector' in lets[n]:
            if 'len' in lets[n]['charselector']:
                l = lets[n]['charselector']['len']
                try:
                    l = str(eval(l, {}, params))
                except Exception:
                    pass
                lets[n]['charselector']['len'] = l

        if 'kindselector' in lets[n]:
            if 'kind' in lets[n]['kindselector']:
                l = lets[n]['kindselector']['kind']
                try:
                    l = str(eval(l, {}, params))
                except Exception:
                    pass
                lets[n]['kindselector']['kind'] = l

        dimension_exprs = {}
        if 'attrspec' in lets[n]:
            attr = lets[n]['attrspec']
            attr.reverse()
            lets[n]['attrspec'] = []
            ...
            for a in attr:
                if a[:9] == 'dimension':
                    ...
                    lets[n] = setattrspec(lets[n], a)
                if intent:
                    if 'intent' not in lets[n]:
                        lets[n]['intent'] = []
                    for c in [x.strip() for x in markoutercomma(intent).split('@,@')]:
                        # Remove spaces so that 'in out' becomes 'inout'
                        tmp = c.replace(' ', '')
                        if tmp not in lets[n]['intent']:
                            lets[n]['intent'].append(tmp)
                    intent = None
                if note:
                    note = note.replace('\\n\\n', '\n\n')
                    note = note.replace('\\n ', '\n')
                    if 'note' not in lets[n]:
                        lets[n]['note'] = [note]
                    else:
                        lets[n]['note'].append(note)
                    note = None
                if depend is not None:
                    if 'depend' not in lets[n]:
                        lets[n]['depend'] = []
                    for c in rmbadname([x.strip() for x in markoutercomma(depend).split('@,@')]):
                        if c not in lets[n]['depend']:
                            lets[n]['depend'].append(c)
                    depend = None
                if check is not None:
                    if 'check' not in lets[n]:
                        lets[n]['check'] = []
                    for c in [x.strip() for x in markoutercomma(check).split('@,@')]:
                        if c not in lets[n]['check']:
                            lets[n]['check'].append(c)
                    check = None
            if dim and 'dimension' not in lets[n]:
                lets[n]['dimension'] = []
                for d in rmbadname(
                    ...
                    try:
                        # the dimension for this letiable depends on a
                        # previously defined parameter
                        d = param_parse(d, params)
                    except (ValueError, IndexError, KeyError):
                        outmess(
                            'analyzelets: could not parse dimension for '
                            f'letiable {d!r}\n'
                        )
                    ...
                    if len(dl) == 2:
                        ...
                        # find letiables v that define d as a linear
                        ...
                        # not defined in block['lets']. Here we assume
                    ...
                    lets[n]['dimension'].append(d)

        if 'check' not in lets[n] and 'args' in block and n in block['args']:
            ...
            n_deps = lets[n].get('depend', [])
            n_checks = []
            n_is_input = l_or(isintent_in, isintent_inout,
                              isintent_inplace)(lets[n])
            if isarray(lets[n]):  # n is array
                for i, d in enumerate(lets[n]['dimension']):
                    ...
                    if coeffs_and_deps is None:
                        ...
                        # may define letiables used in dimension
                        # specifications.
                        for v, (solver, deps) in coeffs_and_deps.items():
                            ...
                            if solver is not None and v not in all_deps:
                                ...
                                # letiables in solver or d.
                                lets[v]['depend'] = [n] + deps
                                if 'check' not in lets[v]:
                                ...
                                is_required = True
                                if 'intent' not in lets[v]:
                                    lets[v]['intent'] = []
                                if 'in' not in lets[v]['intent']:
                                    lets[v]['intent'].append('in')
                                # v needs to be initialized before n
                                n_deps.append(v)
                                n_checks.append(
                                    f'shape({n}, {i}) == {d}')
                            v_attr = lets[v].get('attrspec', [])
                            if not ('optional' in v_attr
                                    or 'required' in v_attr):
                                v_attr.append(
                                    'required' if is_required else 'optional')
                            if v_attr:
                                lets[v]['attrspec'] = v_attr
                    if coeffs_and_deps is not None:
                        # extend v dependencies with ones specified in attrspec
                        for v, (solver, deps) in coeffs_and_deps.items():
                            v_deps = lets[v].get('depend', [])
                            for aa in lets[v].get('attrspec', []):
                                if aa.startswith('depend'):
                                    aa = ''.join(aa.split())
                                    v_deps.extend(aa[7:-1].split(','))
                            if v_deps:
                                lets[v]['depend'] = list(set(v_deps))
                            if n not in v_deps:
                                n_deps.append(v)
            elif isstring(lets[n]):
                if 'charselector' in lets[n]:
                    if '*' in lets[n]['charselector']:
                        length = _eval_length(lets[n]['charselector']['*'],
                                              params)
                        lets[n]['charselector']['*'] = length
                    elif 'len' in lets[n]['charselector']:
                        length = _eval_length(lets[n]['charselector']['len'],
                                              params)
                        del lets[n]['charselector']['len']
                        lets[n]['charselector']['*'] = length
            if n_checks:
                lets[n]['check'] = n_checks
            if n_deps:
                lets[n]['depend'] = list(set(n_deps))

        if '=' in lets[n]:
            if 'attrspec' not in lets[n]:
                lets[n]['attrspec'] = []
            if ('optional' not in lets[n]['attrspec']) and \
               ('required' not in lets[n]['attrspec']):
                lets[n]['attrspec'].append('optional')
            if 'depend' not in lets[n]:
                lets[n]['depend'] = []
                for v, m in list(dep_matches.items()):
                    if m(lets[n]['=']):
                        lets[n]['depend'].append(v)
                if not lets[n]['depend']:
                    del lets[n]['depend']
            if isscalar(lets[n]):
                lets[n]['='] = _eval_scalar(lets[n]['='], params)

    for n in list(lets.keys()):
        if n == block['name']:  # n is block name
            if 'note' in lets[n]:
                block['note'] = lets[n]['note']
            if block['block'] == 'function':
                if 'result' in block and block['result'] in lets:
                    lets[n] = appenddecl(lets[n], lets[block['result']])
                if 'prefix' in block:
                    pr = block['prefix']
                    pr1 = pr.replace('pure', '')
                    ispure = (not pr == pr1)
                    ...
                    if m:
                        ...
                        kindselect, charselect, typename = cracktypespec(
                            typespec, selector)
                        lets[n]['typespec'] = typespec
                        try:
                            if block['result']:
                                lets[block['result']]['typespec'] = typespec
                        except Exception:
                            pass
                        if kindselect:
                            if 'kind' in kindselect:
                                try:
                                    ...
                                    pass
                            lets[n]['kindselector'] = kindselect
                        if charselect:
                            lets[n]['charselector'] = charselect
                        if typename:
                            lets[n]['typename'] = typename
                        if ispure:
                            lets[n] = setattrspec(lets[n], 'pure')
                        if isrec:
                            lets[n] = setattrspec(lets[n], 'recursive')
                    else:
                        outmess(
                            'analyzelets: prefix (%s) were not used\n' % repr(block['prefix']))
    if block['block'] not in ['module', 'pythonmodule', 'python module', 'block data']:
        if 'commonlets' in block:
            neededlets = copy.copy(block['args'] + block['commonlets'])
        else:
            neededlets = copy.copy(block['args'])
        for n in list(lets.keys()):
            if l_or(isintent_callback, isintent_aux)(lets[n]):
                neededlets.append(n)
        if 'entry' in block:
            neededlets.extend(list(block['entry'].keys()))
            for k in list(block['entry'].keys()):
                for n in block['entry'][k]:
                    if n not in neededlets:
                        neededlets.append(n)
        if block['block'] == 'function':
            if 'result' in block:
                neededlets.append(block['result'])
            else:
                neededlets.append(block['name'])
        if block['block'] in ['subroutine', 'function']:
            name = block['name']
            if name in lets and 'intent' in lets[name]:
                block['intent'] = lets[name]['intent']
        if block['block'] == 'type':
            neededlets.extend(list(lets.keys()))
        for n in list(lets.keys()):
            if n not in neededlets:
                del lets[n]
    return lets
...
def param_parse(d, params):
    ...
    Parses the declaration of an array letiable or parameter
...
def expr2name(a, block, args=[]):
    ...
    if a_is_expr:  # `a` is an expression
        implicitrules, attrrules = buildimplicitrules(block)
        at = determineexprtype(a, block['lets'], implicitrules)
        na = 'e_'
        for c in a:
            c = c.lower()
        ...
        if na[-1] == '_':
            ...
            na = na + '_e'
        a = na
        while a in block['lets'] or a in block['args']:
            a = a + 'r'
    if a in args:
        k = 1
    ...
    if a_is_expr:
        ...
        if a not in block['lets']:
            if orig_a in block['lets']:
                block['lets'][a] = block['lets'][orig_a]
            else:
                block['lets'][a] = {}
        if 'externals' in block and orig_a in block['externals'] + block['interfaced']:
            block['lets'][a] = setattrspec(block['lets'][a], 'external')
    return a
...
def analyzeargs(block):
    ...
    if 'result' in block and block['result'] not in block['lets']:
        block['lets'][block['result']] = {}
    return block
...
def determineexprtype(expr, lets, rules={}):
    if expr in lets:
        return _ensure_exprdict(lets[expr])
    expr = expr.strip()
    ...
    for op in ['+', '-', '*', '/']:
        for e in [x.strip() for x in markoutercomma(expr, comma=op).split('@' + op + '@')]:
            if e in lets:
                return _ensure_exprdict(lets[e])
    t = {}
    if determineexprtype_re_4.match(expr):  # in parenthesis
        t = determineexprtype(expr[1:-1], lets, rules)
    else:
        m = determineexprtype_re_5.match(expr)
        if m:
            rn = m.group('name')
            t = determineexprtype(m.group('name'), lets, rules)
...
def crack2fortrangen(block, tab='\n', as_interface=False):
    ...
    if 'args' in block:
        ...
        for a in block['args']:
            ...
            if not isintent_callback(lets[a]):
                argsl.append(a)
    ...
    lets = lets2fortran(
        block, block['lets'], argsl, tab + tabchar, as_interface=as_interface)
    mess = ''
    ...
    ret = '%s%s%s %s%s%s %s%s%s%s%s%s%send %s %s' % (
        tab, prefix, blocktype, name, args, result, mess, f2pyenhancements, use, lets, common, body, tab, blocktype, name)
    return ret
...
def lets2fortran(block, lets, args, tab='', as_interface=False):
    setmesstext(block)
    ret = ''
    nout = []
    for a in args:
        if a in block['lets']:
            nout.append(a)
    if 'commonlets' in block:
        for a in block['commonlets']:
            if a in lets:
                if a not in nout:
                    nout.append(a)
            else:
                errmess(
                    'lets2fortran: Confused?!: "%s" is not defined in lets.\n' % a)
    if 'letnames' in block:
        nout.extend(block['letnames'])
    if not as_interface:
        for a in list(lets.keys()):
            if a not in nout:
                nout.append(a)
    for a in nout:
        if 'depend' in lets[a]:
            for d in lets[a]['depend']:
                if d in lets and 'depend' in lets[d] and a in lets[d]['depend']:
                    errmess(
                        'lets2fortran: Warning: cross-dependence between letiables "%s" and "%s"\n' % (a, d))
        if 'externals' in block and a in block['externals']:
            if isintent_callback(lets[a]):
                ret = '%s%sintent(callback) %s' % (ret, tab, a)
            ret = '%s%sexternal %s' % (ret, tab, a)
            if isoptional(lets[a]):
                ret = '%s%soptional %s' % (ret, tab, a)
            if a in lets and 'typespec' not in lets[a]:
                continue
            cont = 1
            for b in block['body']:
        ...
        if a not in lets:
            show(lets)
            outmess('lets2fortran: No definition for argument "%s".\n' % a)
            continue
        if a == block['name']:
            if block['block'] != 'function' or block.get('result'):
                # 1) skip declaring a letiable that name matches with
                #    subroutine name
                # 2) skip declaring function when its type is
                #    declared via `result` construction
                continue
        if 'typespec' not in lets[a]:
            if 'attrspec' in lets[a] and 'external' in lets[a]['attrspec']:
                if a in args:
                    ret = '%s%sexternal %s' % (ret, tab, a)
                continue
            show(lets[a])
            outmess('lets2fortran: No typespec for argument "%s".\n' % a)
            continue
        ...
        if '*' in selector:
            ...
            if 'len' in selector:
                ...
                letdef = '%s(kind=%s)' % (letdef, selector['kind'])
        c = ' '
        if 'attrspec' in lets[a]:
            attr = [l for l in lets[a]['attrspec']
            ...
            if attr:
                letdef = '%s, %s' % (letdef, ','.join(attr))
                c = ','
        if 'dimension' in lets[a]:
            letdef = '%s%sdimension(%s)' % (
                letdef, c, ','.join(lets[a]['dimension']))
            c = ','
        if 'intent' in lets[a]:
            lst = true_intent_list(lets[a])
            if lst:
                letdef = '%s%sintent(%s)' % (letdef, c, ','.join(lst))
            c = ','
        if 'check' in lets[a]:
            letdef = '%s%scheck(%s)' % (letdef, c, ','.join(lets[a]['check']))
            c = ','
        if 'depend' in lets[a]:
            letdef = '%s%sdepend(%s)' % (
                letdef, c, ','.join(lets[a]['depend']))
            c = ','
        if '=' in lets[a]:
            v = lets[a]['=']
            if lets[a]['typespec'] in ['complex', 'double complex']:
                try:
                    v = eval(v)
                    v = '(%s,%s)' % (v.real, v.imag)
                except Exception:
                    pass
            letdef = '%s :: %s=%s' % (letdef, a, v)
        else:
            letdef = '%s :: %s' % (letdef, a)
        ret = '%s%s%s' % (ret, tab, letdef)
    return ret
######


# We expose post_processing_hooks as global letiable so that
...
def character_backward_compatibility_hook(item, parents, result,
    ...
    letiables in `check`, `dimension`, `=`, and `callstatement`
    expressions.
    ...
    def fix_usage(letname, value):
        value = re.sub(r'[*]\s*\b' + letname + r'\b', letname, value)
        value = re.sub(r'\b' + letname + r'\b\s*[\[]\s*0\s*[\]]',
                       letname, value)
        return value

    if parent_key in ['dimension', 'check']:
        assert parents[-3][0] == 'lets'
        lets_dict = parents[-3][1]
    elif key == '=':
        assert parents[-2][0] == 'lets'
        lets_dict = parents[-2][1]
    else:
        lets_dict = None

    new_value = None
    if lets_dict is not None:
        new_value = value
        for letname, vd in lets_dict.items():
            if ischaracter(vd):
                new_value = fix_usage(letname, new_value)
    elif key == 'callstatement':
        lets_dict = parents[-2][1]['lets']
        new_value = value
        for letname, vd in lets_dict.items():
            if ischaracter(vd):
                # replace all occurrences of `<letname>` with
                # `&<letname>` in argument passing
                new_value = re.sub(
                    r'(?<![&])\b' + letname + r'\b', '&' + letname, new_value)
