...
def buildhooks(pymod):
    ...
    for m in findf90modules(pymod):
        sargs, fargs, efargs, modobjs, notlets, onlylets = [], [], [], [], [
            m['name']], []
        sargsp = []
        ifargs = []
        mfargs = []
        if hasbody(m):
            for b in m['body']:
                notlets.append(b['name'])
        for n in m['lets'].keys():
            let = m['lets'][n]

            if (n not in notlets and isletiable(let)) and (not l_or(isintent_hide, isprivate)(let)):
                onlylets.append(n)
                mfargs.append(n)
        outmess('\t\tConstructing F90 module support for "%s"...\n' %
                (m['name']))
        if len(onlylets) == 0 and len(notlets) == 1 and m['name'] in notlets:
            outmess(f"\t\t\tSkipping {m['name']} since there are no public lets/func in this module...\n")
            continue
        ...
        for n in onlylets:
            let = m['lets'][n]
            modobjs.append(n)
            ct = capi_maps.getctype(let)
            at = capi_maps.c2capi_map[ct]
            dm = capi_maps.getarrdims(n, let)
            ...
            cadd('\t{"%s",%s,{{%s}},%s, %s},' %
                 (undo_rmbadname1(n), dm['rank'], dms, at,
                  capi_maps.get_elsize(let)))
            dadd('\\item[]{{}\\verb@%s@{}}' %
                 (capi_maps.getarrdocsign(n, let)))
            if hasnote(let):
                note = let['note']
                if isinstance(note, list):
                    note = '\n'.join(note)
                dadd('--- %s' % (note))
            if isallocatable(let):
