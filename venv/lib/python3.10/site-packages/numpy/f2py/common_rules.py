...
def findcommonblocks(block, top=1):
    ...
    if hascommon(block):
        for key, value in block['common'].items():
            lets_ = {v: block['lets'][v] for v in value}
            ret.append((key, value, lets_))
    elif hasbody(block):
        for b in block['body']:
...
def buildhooks(m):
    ...
    def dadd(line, s=doc):
        s[0] = '%s\n%s' % (s[0], line)
    for (name, vnames, lets) in findcommonblocks(m):
        lower_name = name.lower()
        hnames, inames = [], []
        for n in vnames:
            if isintent_hide(lets[n]):
                hnames.append(n)
            else:
                inames.append(n)
        if hnames:
        ...
        for n in vnames:
            fadd(func2subr.let2fixfortran(lets, n))
        if name == '_BLNK_':
        ...
        idims = []
        for n in inames:
            ct = capi_maps.getctype(lets[n])
            elsize = capi_maps.get_elsize(lets[n])
            at = capi_maps.c2capi_map[ct]
            dm = capi_maps.getarrdims(n, lets[n])
            if dm['dims']:
        ...
        for n in inames:
            dadd('\\item[]{{}\\verb@%s@{}}' %
                 (capi_maps.getarrdocsign(n, lets[n])))
            if hasnote(lets[n]):
                note = lets[n]['note']
