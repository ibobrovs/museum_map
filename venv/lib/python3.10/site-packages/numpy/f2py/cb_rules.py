...
cb_arg_rules = [
    {  # Doc
        ...
        'docsign': {l_and(isrequired, isintent_nothide): '#letname#,'},
        'docsignopt': {l_and(isoptional, isintent_nothide): '#letname#,'},
        'depend': ''
    },
    {
        'args': {
            l_and(isscalar, isintent_c): '#ctype# #letname_i#',
            l_and(isscalar, l_not(isintent_c)): '#ctype# *#letname_i#_cb_capi',
            isarray: '#ctype# *#letname_i#',
            isstring: '#ctype# #letname_i#'
        },
        'args_nm': {
            l_and(isscalar, isintent_c): '#letname_i#',
            l_and(isscalar, l_not(isintent_c)): '#letname_i#_cb_capi',
            isarray: '#letname_i#',
            isstring: '#letname_i#'
        },
        'args_td': {
        ...
        'strarglens': {isstring: ',int #letname_i#_cb_len'},
        'strarglens_td': {isstring: ',int'},  # untested with multiple args
        # untested with multiple args
        'strarglens_nm': {isstring: ',#letname_i#_cb_len'},
    },
    {  # Scalars
        'decl': {l_not(isintent_c): '    #ctype# #letname_i#=(*#letname_i#_cb_capi);'},
        'error': {l_and(isintent_c, isintent_out,
                        throw_error('intent(c,out) is forbidden for callback scalar arguments')):
                  ''},
        'frompyobj': [{debugcapi: '    CFUNCSMESS("cb:Getting #letname#->");'},
                      {isintent_out:
                       '    if (capi_j>capi_i)\n        GETSCALARFROMPYTUPLE(capi_return,capi_i++,#letname_i#_cb_capi,#ctype#,"#ctype#_from_pyobj failed in converting argument #letname# of call-back function #name# to C #ctype#\\n");'},
                      ...
                      {l_and(debugcapi, l_and(iscomplex, isintent_c)):
                          '    fprintf(stderr,"#showvalueformat#.\\n",(#letname_i#).r,(#letname_i#).i);'},
                      {l_and(debugcapi, l_and(iscomplex, l_not( isintent_c))):
                          '    fprintf(stderr,"#showvalueformat#.\\n",(*#letname_i#_cb_capi).r,(*#letname_i#_cb_capi).i);'},
                      ],
    ...
    if (cb->nofargs>capi_i)
        if (CAPI_ARGLIST_SETITEM(capi_i++,pyobj_from_#ctype#1(#letname_i#)))
            goto capi_fail;"""},
                      {isintent_inout: """\
    if (cb->nofargs>capi_i)
        if (CAPI_ARGLIST_SETITEM(capi_i++,pyarr_from_p_#ctype#1(#letname_i#_cb_capi)))
            goto capi_fail;"""}],
        ...
        'frompyobj': [{debugcapi: '    CFUNCSMESS("cb:Getting #letname#->\\"");'},
                      """    if (capi_j>capi_i)
        GETSTRFROMPYTUPLE(capi_return,capi_i++,#letname_i#,#letname_i#_cb_len);""",
                      {debugcapi:
                       '    fprintf(stderr,"#showvalueformat#\\":%d:.\\n",#letname_i#,#letname_i#_cb_len);'},
                      ],
        ...
        'pyobjfrom': [
            {debugcapi:
             ('    fprintf(stderr,"debug-capi:cb:#letname#=#showvalueformat#:'
              '%d:\\n",#letname_i#,#letname_i#_cb_len);')},
            {isintent_in: """\
    if (cb->nofargs>capi_i)
        if (CAPI_ARGLIST_SETITEM(capi_i++,pyobj_from_#ctype#1size(#letname_i#,#letname_i#_cb_len)))
            goto capi_fail;"""},
                      {isintent_inout: """\
    if (cb->nofargs>capi_i) {
        int #letname_i#_cb_dims[] = {#letname_i#_cb_len};
        if (CAPI_ARGLIST_SETITEM(capi_i++,pyarr_from_p_#ctype#1(#letname_i#,#letname_i#_cb_dims)))
            goto capi_fail;
    }"""}],
    ...
    # Array ...
    {
        'decl': '    npy_intp #letname_i#_Dims[#rank#] = {#rank*[-1]#};',
        'setdims': '    #cbsetdims#;',
        '_check': isarray,
        '_depend': ''
    },
    {
        'pyobjfrom': [{debugcapi: '    fprintf(stderr,"debug-capi:cb:#letname#\\n");'},
                      {isintent_c: """\
    if (cb->nofargs>capi_i) {
        ...
        PyArrayObject *tmp_arr = (PyArrayObject *)PyArray_New(&PyArray_Type,
          #rank#,#letname_i#_Dims,#atype#,NULL,(char*)#letname_i#,#elsize#,
...
""",
        ...
        rv_cb_arr =  array_from_pyobj(#atype#,#letname_i#_Dims,#rank#,F2PY_INTENT_IN""",
        ...
        MEMCOPY(#letname_i#,PyArray_DATA(rv_cb_arr),PyArray_NBYTES(rv_cb_arr));
...
def buildcallback(rout, um):
    ...
    for i, a in enumerate(args):
        vrd = capi_maps.cb_sign2map(a, let[a], index=i)
        savevrd[a] = vrd
        for r in cb_arg_rules:
            if '_depend' in r:
                continue
            if '_optional' in r and isoptional(let[a]):
                continue
    ...
    for a in args:
        ...
        for r in cb_arg_rules:
            if '_depend' in r:
                continue
            if ('_optional' not in r) or ('_optional' in r and isrequired(let[a])):
                continue
            if ('_check' in r and r['_check'](let[a])) or ('_check' not in r):
                ar = applyrules(r, vrd, let[a])
                rd = dictappend(rd, ar)
