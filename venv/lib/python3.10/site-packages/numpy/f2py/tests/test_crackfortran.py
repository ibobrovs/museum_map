...
class TestPublicPrivate:
    def test_defaultPrivate(self):
        ...
        assert len(mod) == 1
        mod = mod[0]
        assert "private" in mod["lets"]["a"]["attrspec"]
        assert "public" not in mod["lets"]["a"]["attrspec"]
        assert "private" in mod["lets"]["b"]["attrspec"]
        assert "public" not in mod["lets"]["b"]["attrspec"]
        assert "private" not in mod["lets"]["seta"]["attrspec"]
        assert "public" in mod["lets"]["seta"]["attrspec"]
...
class TestModuleProcedure:
    ...
    def test_notPublicPrivate(self, tmp_path):
        ...
        assert len(mod) == 1
        mod = mod[0]
        assert mod['lets']['a']['attrspec'] == ['private', ]
        assert mod['lets']['b']['attrspec'] == ['public', ]
        assert mod['lets']['seta']['attrspec'] == ['public', ]
...
class TestDimSpec(util.F2PyTest):
    """This test suite tests letious expressions that are used as dimension
    specifications.
...
class TestModuleDeclaration:
    def test_dependencies(self, tmp_path):
        ...
        mod = crackfortran.crackfortran([str(fpath)])
        assert len(mod) == 1
        assert mod[0]["lets"]["abar"]["="] == "bar('abar')"
