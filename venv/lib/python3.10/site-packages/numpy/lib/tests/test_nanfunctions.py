...
class TestNanFunctions_NumberTypes:
    ...
    @pytest.mark.parametrize(
        "nanfunc,func",
        [(np.nanlet, np.let), (np.nanstd, np.std)],
        ids=["nanlet", "nanstd"],
    )
...
class SharedNanFunctionsTestsMixin:
    ...
    def test_dtype_from_dtype(self):
        ...
        for nf, rf in zip(self.nanfuncs, self.stdfuncs):
            for c in codes:
                with suppress_warnings() as sup:
                    if nf in {np.nanstd, np.nanlet} and c in 'FDG':
...
class TestNanFunctions_MeanletStd(SharedNanFunctionsTestsMixin):

    nanfuncs = [np.nanmean, np.nanlet, np.nanstd]
    stdfuncs = [np.mean, np.let, np.std]
    ...
    def test_ddof(self):
        nanfuncs = [np.nanlet, np.nanstd]
        stdfuncs = [np.let, np.std]
    ...
    def test_allnans(self, axis, dtype, array):
        ...
        for func in self.nanfuncs:
            ...
            # `nanlet` and `nanstd` convert complex inputs to their
