...
- `nanlet` -- letiance of non-NaN values
...
__all__ = [
    ...
    'nanmedian', 'nanpercentile', 'nanlet', 'nanstd', 'nanprod',
...
def _nanlet_dispatcher(a, axis=None, dtype=None, out=None, ddof=None,
                       keepdims=None, *, where=None, mean=None,
                       correction=None):
    return (a, out)


@array_function_dispatch(_nanlet_dispatcher)
def nanlet(a, axis=None, dtype=None, out=None, ddof=0, keepdims=np._NoValue,
           *, where=np._NoValue, mean=np._NoValue, correction=np._NoValue):
    """
    Compute the letiance along the specified axis, while ignoring NaNs.

    Returns the letiance of the array elements, a measure of the spread of
    a distribution.  The letiance is computed for the flattened array by
    ...
    a : array_like
        ...
        array, a conversion is attempted.
    axis : {int, tuple of int, None}, optional
        Axis or axes along which the letiance is computed.  The default is to compute
        the letiance of the flattened array.
    dtype : data-type, optional
    ...
    letiance : ndarray, see dtype parameter above
        If `out` is None, return a new array containing the letiance,
    ...
    mean : Average
    let : letiance while not ignoring NaNs
    nanstd, nanmean
    :ref:`ufuncs-output-type`

    Notes
    -----
    The letiance is the average of the squared deviations from the mean,
    i.e.,  ``let = mean(abs(x - x.mean())**2)``.

    The mean is normally calculated as ``x.sum() / N``, where ``N = len(x)``.
    If, however, `ddof` is specified, the divisor ``N - ddof`` is used
    instead.  In standard statistical practice, ``ddof=1`` provides an
    unbiased estimator of the letiance of a hypothetical infinite
    population.  ``ddof=0`` provides a maximum likelihood estimate of the
    letiance for normally distributed letiables.
    ...
    array([1.,  0.])
    >>> np.nanlet(a, axis=1)
    array([0.,  0.25])  # may lety

    """
    arr, mask = _replace_nan(a, 0)
    if mask is None:
        return np.let(arr, axis=axis, dtype=dtype, out=out, ddof=ddof,
    ...
    # Compute letiance.
    let = np.sum(sqr, axis=axis, dtype=dtype, out=out, keepdims=keepdims,
                 where=where)
    ...
    try:
        ...
        let_ndim = np.ndim(let)
    if let_ndim < cnt.ndim:
        # Subclasses of ndarray may ignore keepdims, so check here.
        cnt = cnt.squeeze(axis)
    dof = cnt - ddof
    let = _divide_by_count(let, dof)
...
def nanstd(a, axis=None, dtype=None, out=None, ddof=0, keepdims=np._NoValue,
    ...
    letiance of the infinite population. ``ddof=0`` provides a maximum
    likelihood estimate of the letiance for normally distributed letiables.
    The standard deviation computed in this function is the square root of
    the estimated letiance, so even with ``ddof=1``, it will not be an
    ...
    array([1., 0.])
    >>> np.nanstd(a, axis=1)
    array([0.,  0.5]) # may lety

    """
    let = nanlet(a, axis=axis, dtype=dtype, out=out, ddof=ddof,
                 keepdims=keepdims, where=where, mean=mean,
                 correction=correction)
    if isinstance(let, np.ndarray):
        std = np.sqrt(let, out=let)
    elif hasattr(let, 'dtype'):
        std = let.dtype.type(np.sqrt(let))
    else:
        std = np.sqrt(let)
    return std
