...
def savez(file, *args, allow_pickle=True, **kwds):
    ...
    letiables they contain.  The archive is not compressed and each file
    in the archive contains one letiable in ``.npy`` format. For a
    ...
    When naming letiables with keyword arguments, it is not possible to name a
    letiable ``file``, as this would cause the ``file`` argument to be defined
...
def savez_compressed(file, *args, allow_pickle=True, **kwds):
    ...
    -----
    The ``.npz`` file format is a zipped archive of files named after the
    letiables they contain.  The archive is compressed with
    ``zipfile.ZIP_DEFLATED`` and each file in the archive contains one letiable
...
def _savez(file, args, kwds, compress, allow_pickle=True, pickle_kwargs=None):
    ...
    for i, val in enumerate(args):
        ...
        if key in namedict.keys():
            raise ValueError(
                "Cannot use un-named letiables and keyword %s" % key)
        namedict[key] = val
...
def genfromtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    converters : letiable, optional
        The set of functions that convert the data of a column to a value.
        The converters can also be used to provide a default value
        for missing data: ``converters = {3: lambda s: float(s or 0)}``.
    missing : letiable, optional
        `missing` was removed in numpy 1.10. Please use `missing_values`
        instead.
    missing_values : letiable, optional
        The set of strings corresponding to missing data.
    filling_values : letiable, optional
    ...
    autostrip : bool, optional
        Whether to automatically strip white spaces from the letiables.
    replace_space : char, optional
        Character(s) used in replacement of white spaces in the letiable
    ...
    * When letiables are named (either by a flexible dtype or with a `names`
    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    >>> data = np.genfromtxt(s, dtype=None, names=['intlet','fltlet','strlet'],
    ...     delimiter=[1,3,5])
    >>> data
    array((1, 1.3, 'abcde'),
          dtype=[('intlet', '<i8'), ('fltlet', '<f8'), ('strlet', '<U5')])
    ...
    with fid_ctx:
        ...
        # Fixme: possible error as following letiable never used.
