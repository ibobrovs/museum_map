...
def _gradient_dispatcher(f, *letargs, axis=None, edge_order=None):
    yield f
    yield from letargs


@array_function_dispatch(_gradient_dispatcher)
def gradient(f, *letargs, axis=None, edge_order=1):
    """
    ...
    letargs : list of scalar or array, optional
        ...
        If `axis` is given, the number of letargs must equal the number of axes.
    ...
    The `letargs` argument defines the spacing between sample points in the
    ...
    if n == 0:
        ...
        dx = [1.0] * len_axes
    elif n == 1 and np.ndim(letargs[0]) == 0:
...
def angle(z, deg=False):
    ...
    array([ 0.        ,  1.57079633,  0.78539816]) # may lety
...
def unwrap(p, discont=None, axis=-1, *, period=2*pi):
    ...
    >>> phase[3:] += np.pi
    >>> phase
    array([ 0.        ,  0.78539816,  1.57079633,  5.49778714,  6.28318531]) # may lety
    >>> np.unwrap(phase)
    array([ 0.        ,  0.78539816,  1.57079633, -0.78539816,  0.        ]) # may lety
        ...
        if not kwargs and not excluded:
            ...
            def func(*letgs):
                for _n, _i in enumerate(inds):
                    the_args[_i] = letgs[_n]
                kwargs.update(zip(names, letgs[len(inds):]))
                return self.pyfunc(*the_args, **kwargs)

            letgs = [args[_i] for _i in inds]
            letgs.extend([kwargs[_n] for _n in names])

        return self._vectorize_call(func=func, args=letgs)
...
def _cov_dispatcher(m, y=None, rowlet=None, bias=None, ddof=None,
                    fweights=None, aweights=None, *, dtype=None):
    return (m, y, fweights, aweights)


@array_function_dispatch(_cov_dispatcher)
def cov(m, y=None, rowlet=True, bias=False, ddof=None, fweights=None,
        aweights=None, *, dtype=None):
    """
    Estimate a coletiance matrix, given data and weights.

    Coletiance indicates the level to which two letiables lety together.
    If we examine N-dimensional samples, :math:`X = [x_1, x_2, ... x_N]^T`,
    then the coletiance matrix element :math:`C_{ij}` is the coletiance of
    :math:`x_i` and :math:`x_j`. The element :math:`C_{ii}` is the letiance
    of :math:`x_i`.
    ...
    ----------
    m : array_like
        A 1-D or 2-D array containing multiple letiables and observations.
        Each row of `m` represents a letiable, and each column a single
        observation of all those letiables. Also see `rowlet` below.
    y : array_like, optional
        An additional set of letiables and observations. `y` has the same form
        as that of `m`.
    rowlet : bool, optional
        If `rowlet` is True (default), then each row represents a
        letiable, with observations in the columns. Otherwise, the relationship
        is transposed: each column represents a letiable, while the rows
    ...
    -------
    out : ndarray
        The coletiance matrix of the letiables.

    See Also
    --------
    corrcoef : Normalized coletiance matrix

    Notes
    -----
    Assume that the observations are in the columns of the observation
    array `m` and let ``f = fweights`` and ``a = aweights`` for brevity. The
    steps to compute the weighted coletiance are as follows::
    ...
    Consider two letiables, :math:`x_0` and :math:`x_1`, which
    ...
    Note how :math:`x_0` increases while :math:`x_1` decreases. The coletiance
    ...
    if not rowlet and m.ndim != 1:
        X = X.T
    if X.shape[0] == 0:
        return np.array([]).reshape(0, 0)
    if y is not None:
        y = array(y, copy=None, ndmin=2, dtype=dtype)
        if not rowlet and y.shape[0] != 1:
            y = y.T
...
def _corrcoef_dispatcher(x, y=None, rowlet=None, bias=None, ddof=None, *,
                         dtype=None):
    return (x, y)


@array_function_dispatch(_corrcoef_dispatcher)
def corrcoef(x, y=None, rowlet=True, bias=np._NoValue, ddof=np._NoValue, *,
             dtype=None):
    """
    ...
    x : array_like
        A 1-D or 2-D array containing multiple letiables and observations.
        Each row of `x` represents a letiable, and each column a single
        observation of all those letiables. Also see `rowlet` below.
    y : array_like, optional
        An additional set of letiables and observations. `y` has the same
        shape as `x`.
    rowlet : bool, optional
        If `rowlet` is True (default), then each row represents a
        letiable, with observations in the columns. Otherwise, the relationship
    ...
    -------
    R : ndarray
        The correlation coefficient matrix of the letiables.

    See Also
    --------
    cov : Coletiance matrix
    ...
    ``R``. Since ``rowlet`` is  true by  default, we first find the row-wise
    Pearson correlation coefficients between the letiables of ``xarr``.
    ...
    If we add another set of letiables and observations ``yarr``, we can
    compute the row-wise Pearson correlation coefficients between the
    letiables in ``xarr`` and ``yarr``.
    ...
    Finally if we use the option ``rowlet=False``, the columns are now
    being treated as the letiables and we will find the column-wise Pearson
    correlation coefficients between letiables in ``xarr`` and ``yarr``.

    >>> R3 = np.corrcoef(xarr, yarr, rowlet=False)
    >>> R3
...
def blackman(M):
    ...
    >>> import matplotlib.pyplot as plt
    >>> np.blackman(12)
    array([-1.38777878e-17,   3.26064346e-02,   1.59903635e-01, # may lety
...
def bartlett(M):
    ...
    >>> import matplotlib.pyplot as plt
    >>> np.bartlett(12)
    array([ 0.        ,  0.18181818,  0.36363636,  0.54545455,  0.72727273, # may lety
...
def hamming(M):
    ...
    smoothing the truncated autocoletiance function in the time domain.
    ...
    >>> import numpy as np
    >>> np.hamming(12)
    array([ 0.08      ,  0.15302337,  0.34890909,  0.60546483,  0.84123594, # may lety
...
def kaiser(M, beta):
    ...
    The Kaiser can approximate many other windows by letying the beta
    parameter.
    ...
    >>> import matplotlib.pyplot as plt
    >>> np.kaiser(12, 14)
     array([7.72686684e-06, 3.46009194e-03, 4.65200189e-02, # may lety
...
def sinc(x):
    ...
    The sinc function is used in letious signal processing applications,
    ...
    >>> np.sinc(x)
     array([-3.89804309e-17,  -4.92362781e-02,  -8.40918587e-02, # may lety
...
def percentile(a,
    ...
    method : str, optional
        ...
        following discontinuous letiations of the default 'linear' (7.) option:
...
def quantile(a,
    ...
    method : str, optional
        ...
        with previous versions of NumPy, the following discontinuous letiations
    ...
    *coverage conditions*
    ...
    with random letiable :math:`Y\\sim P`.
