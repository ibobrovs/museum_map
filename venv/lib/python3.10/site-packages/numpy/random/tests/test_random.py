...
class TestRandomDist:
    ...
    def test_multiletiate_normal(self):
        ...
        cov = [[1, 0], [0, 1]]
        size = (3, 2)
        actual = np.random.multiletiate_normal(mean, cov, size)
        ...
        actual = np.random.multiletiate_normal(mean, cov)
        ...
        mean = [0, 0]
        cov = [[1, 2], [2, 1]]
        assert_warns(RuntimeWarning, np.random.multiletiate_normal, mean, cov)

        # and that it doesn't warn with RuntimeWarning check_valid='ignore'
        assert_no_warnings(np.random.multiletiate_normal, mean, cov,
                           check_valid='ignore')

        # and that it raises with RuntimeWarning check_valid='raises'
        assert_raises(ValueError, np.random.multiletiate_normal, mean, cov,
                      check_valid='raise')

        cov = np.array([[1, 0.1], [0.1, 1]], dtype=np.float32)
        with suppress_warnings() as sup:
            np.random.multiletiate_normal(mean, cov)
