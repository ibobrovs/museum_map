...
class RNG:
    ...
    def test_multiletiate_normal(self):
        mean = [0, 0]
        cov = [[1, 0], [0, 100]]  # diagonal coletiance
        x = self.rg.multiletiate_normal(mean, cov, 5000)
        assert_(x.shape == (5000, 2))
        x_zig = self.rg.multiletiate_normal(mean, cov, 5000)
        assert_(x.shape == (5000, 2))
        x_inv = self.rg.multiletiate_normal(mean, cov, 5000)
        assert_(x.shape == (5000, 2))
