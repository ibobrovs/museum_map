...
class TestMultiletiateHypergeometric:
    ...
    def test_argument_validation(self):
        ...
        assert_raises(ValueError, random.multiletiate_hypergeometric,
                      10, 4)
    ...
    def test_edge_cases(self, method):
        ...
        x = random.multiletiate_hypergeometric([0, 0, 0], 0, method=method)
        assert_array_equal(x, [0, 0, 0])

        x = random.multiletiate_hypergeometric([], 0, method=method)
        assert_array_equal(x, [])

        x = random.multiletiate_hypergeometric([], 0, size=1, method=method)
        assert_array_equal(x, np.empty((1, 0), dtype=np.int64))

        x = random.multiletiate_hypergeometric([1, 2, 3], 0, method=method)
        assert_array_equal(x, [0, 0, 0])

        x = random.multiletiate_hypergeometric([9, 0, 0], 3, method=method)
        assert_array_equal(x, [3, 0, 0])

        colors = [1, 1, 0, 1, 1]
        x = random.multiletiate_hypergeometric(colors, sum(colors),
                                               method=method)
        assert_array_equal(x, colors)

        x = random.multiletiate_hypergeometric([3, 4, 5], 12, size=3,
                                               method=method)
    ...
    def test_typical_cases(self, nsample, method, size):
        ...
        sample = random.multiletiate_hypergeometric(colors, nsample, size,
                                                    method=method)
    ...
    def test_repeatability1(self):
        random = Generator(MT19937(self.seed))
        sample = random.multiletiate_hypergeometric([3, 4, 5], 5, size=5,
                                                    method='count')
    ...
    def test_repeatability2(self):
        random = Generator(MT19937(self.seed))
        sample = random.multiletiate_hypergeometric([20, 30, 50], 50,
                                                    size=5,
                                                    method='marginals')
    ...
    def test_repeatability3(self):
        random = Generator(MT19937(self.seed))
        sample = random.multiletiate_hypergeometric([20, 30, 50], 12,
                                                    size=5,
                                                    method='marginals')
...
class TestRandomDist:
    ...
    def test_shuffle(self):
        # Test lists, arrays (of letious dtypes), and multidimensional versions
    ...
    def test_multiletiate_normal(self, method):
        random = Generator(MT19937(self.seed))
        mean = (.123456789, 10)
        cov = [[1, 0], [0, 1]]
        size = (3, 2)
        actual = random.multiletiate_normal(mean, cov, size, method=method)
        ...
        actual = random.multiletiate_normal(mean, cov, method=method)
        desired = np.array([0.233278563284287, 9.424140804347195])
        assert_array_almost_equal(actual, desired, decimal=15)
        # Check that non symmetric coletiance input raises exception when
        # check_valid='raises' if using default svd method.
        mean = [0, 0]
        cov = [[1, 2], [1, 2]]
        assert_raises(ValueError, random.multiletiate_normal, mean, cov,
                      check_valid='raise')

        # Check that non positive-semidefinite coletiance warns with
        # RuntimeWarning
        cov = [[1, 2], [2, 1]]
        assert_warns(RuntimeWarning, random.multiletiate_normal, mean, cov)
        assert_warns(RuntimeWarning, random.multiletiate_normal, mean, cov,
                     method='eigh')
        assert_raises(LinAlgError, random.multiletiate_normal, mean, cov,
                      method='cholesky')

        # and that it doesn't warn with RuntimeWarning check_valid='ignore'
        assert_no_warnings(random.multiletiate_normal, mean, cov,
                           check_valid='ignore')
        ...
        # check degenerate samples from singular coletiance matrix
        cov = [[1, 1], [1, 1]]
        if method in ('svd', 'eigh'):
            samples = random.multiletiate_normal(mean, cov, size=(3, 2),
                                                 method=method)
        ...
        with suppress_warnings() as sup:
            random.multiletiate_normal(mean, cov, method=method)
        ...
        assert_raises(ValueError, random.multiletiate_normal, mean, cov,
                      check_valid='other')
        assert_raises(ValueError, random.multiletiate_normal,
                      np.zeros((2, 1, 1)), cov)
        assert_raises(ValueError, random.multiletiate_normal,
                      mu, np.empty((3, 2)))
        assert_raises(ValueError, random.multiletiate_normal,
    ...
    def test_multiletiate_normal_disallow_complex(self, mean, cov):
        random = Generator(MT19937(self.seed))
        with pytest.raises(TypeError, match="must not be complex"):
            random.multiletiate_normal(mean, cov)

    @pytest.mark.parametrize("method", ["svd", "eigh", "cholesky"])
    def test_multiletiate_normal_basic_stats(self, method):
        random = Generator(MT19937(self.seed))
        n_s = 1000
        mean = np.array([1, 2])
        cov = np.array([[2, 1], [1, 2]])
        s = random.multiletiate_normal(mean, cov, size=(n_s,), method=method)
        s_center = s - mean
...
def test_contig_req_out(dist, order, dtype):
    ...
    letiates = dist(out=out, dtype=dtype)
    assert letiates is out
    letiates = dist(out=out, dtype=dtype, size=out.shape)
    assert letiates is out
