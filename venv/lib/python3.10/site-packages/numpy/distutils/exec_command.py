...
takes keyword arguments for (re-)defining environment letiables.
...
Provides functions:
  ...
  find_executable --- locate a command using info from environment
                    letiable PATH. Equivalent to posix `which`
                    command.
...
nt        win32         Cygwin 98-4.10, Python 2.1.1(MSC) - echo tests
                        fail i.e. redefining environment letiables may
                        not work. FIXED: don't use cygwin echo!
                        Comment: also `cmd /c echo` will not work
                        but redefining environment letiables do work.
