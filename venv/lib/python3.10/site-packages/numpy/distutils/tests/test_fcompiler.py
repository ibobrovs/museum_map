...
def test_fcompiler_flags(monkeypatch):
    ...
    flag_lets = fc.flag_lets.clone(lambda *args, **kwargs: None)

    for opt, envlet in customizable_flags:
        new_flag = '-dummy-{}-flag'.format(opt)
        prev_flags = getattr(flag_lets, opt)

        monkeypatch.setenv(envlet, new_flag)
        new_flags = getattr(flag_lets, opt)

        monkeypatch.delenv(envlet)
