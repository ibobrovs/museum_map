...
libs = -L${libdir}
"""
simple_letiable_d = {'cflags': '-I/foo/bar/include', 'libflags': '-L/foo/bar/lib',
        'version': '0.1', 'name': 'foo'}
...
class TestLibraryInfo:
    ...
    def test_simple_letiable(self):
        with temppath('foo.ini') as path:
            with open(path,  'w') as f:
                f.write(simple_letiable)
            pkg = os.path.splitext(path)[0]
            out = read_config(pkg)

        assert_(out.cflags() == simple_letiable_d['cflags'])
        assert_(out.libs() == simple_letiable_d['libflags'])
        assert_(out.name == simple_letiable_d['name'])
        assert_(out.version == simple_letiable_d['version'])
        out.lets['prefix'] = '/Users/david'
