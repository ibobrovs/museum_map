...
def get_num_build_jobs():
    ...
    If the command did not receive a setting the environment letiable
...
def get_shared_lib_extension(is_python_ext=False):
    ...
    """
    conflets = distutils.sysconfig.get_config_lets()
    so_ext = conflets.get('EXT_SUFFIX', '')

    if not is_python_ext:
        # hardcode known values, config lets (including SHLIB_SUFFIX) are
        # unreliable (see #3182)
        ...
        if (sys.platform.startswith('linux') or
            ...
            # fall back to config lets for unknown platforms
            # fix long extension for Python >=3.2, see PEP 3149.
            if 'SOABI' in conflets:
                # Does nothing unless SOABI config let exists
                so_ext = so_ext.replace('.' + conflets.get('SOABI'), '', 1)
...
class Configuration:
    ...
    def add_npy_pkg_config(self, template, install_dir, subst_dict=None):
        ...
        given install directory, using `subst_dict` for letiable substitution.
        ...
        subst_dict : dict, optional
            ...
            prefix is always available through the letiable ``@prefix@``, since the
        ...
        letiable to point to the directory with the modified npy-pkg-config
        files.
    ...
    def get_version(self, version_file=None, version_letiable=None):
        ...
        __svn_version__.py for string letiables version, __version__, and
        ...
        if version_file is None:
            ...
            files = [version_file]
        if version_letiable is None:
            version_lets = ['version',
                            '__version__',
                            self.name.split('.')[-1]+'_version']
        else:
            version_lets = [version_letiable]
        for f in files:
...
def get_npy_pkg_dir():
    ...
    If the NPY_PKG_CONFIG_PATH environment letiable is set, the value of that
    ...
    with open(target, 'w') as f:
        ...
        f.write(textwrap.dedent(r'''
            ...
            def show():
                ...
                Print information about letious resources (libraries, library
                ...
                2. NumPy BLAS/LAPACK Installation Notes
                   ...
                   environment letiables NPY_BLAS_LIBS, NPY_CBLAS_LIBS, and
