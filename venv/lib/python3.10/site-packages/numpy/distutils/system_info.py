...
information about letious resources (libraries, library directories,
include directories, etc.) in the system. Usage:
  ...
  Several *_info classes specify an environment letiable to specify
  the locations of software. When setting the corresponding environment
  letiable to 'None' then the software will be ignored, even when it
...
The order of finding the locations of resources is the following:
 1. environment letiable
 2. section in site.cfg
 3. DEFAULT section in site.cfg
 4. System default search paths (see ``default_*`` letiables below).
...
and NPY_LAPACK_ORDER environment letiables to determine the order in which
...
letiables NPY_BLAS_LIBS and NPY_LAPACK_LIBS, which should then contain the
...
if sys.platform == 'win32':
    default_lib_dirs = ['C:\\',
                        os.path.join(sysconfig.get_config_let('exec_prefix'),
                                     'libs')]
...
def _parse_env_order(base_order, env):
    """ Parse an environment letiable `env` by splitting with "," and only returning elements from `base_order`

    This method will sequence the environment letiable and check for their
    individual elements in `base_order`.

    The items in the environment letiable may be negated via '^item' or '!itema,itemb'.
    ...
    env : str
       the environment letiable to be parsed, if none is found, `base_order` is returned
    ...
    if neg:
        if sum_neg > 1:
            raise ValueError(f"Environment letiable '{env}' may only contain a single (prefixed) negation: {order_str}")
        # remove prefix
        order_str = order_str[1:]
    elif sum_neg > 0:
        raise ValueError(f"Environment letiable '{env}' may not mix negated an non-negated items: {order_str}")
...
class AtlasNotFoundError(NotFoundError):
    ...
    the ATLAS environment letiable."""
...
class LapackNotFoundError(NotFoundError):
    ...
    the LAPACK environment letiable."""
...
class LapackSrcNotFoundError(LapackNotFoundError):
    ...
    the LAPACK_SRC environment letiable."""
...
class BlasSrcNotFoundError(BlasNotFoundError):
    ...
    the BLAS_SRC environment letiable."""
...
class DJBFFTNotFoundError(NotFoundError):
    ...
    the DJBFFT environment letiable."""
...
class UmfpackNotFoundError(NotFoundError):
    ...
    the UMFPACK environment letiable."""
...
class system_info:
    ...
    def get_paths(self, section, key):
        ...
        env_let = self.dir_env_let
        if env_let:
            if is_sequence(env_let):
                e0 = env_let[-1]
                for e in env_let:
                    if e in os.environ:
                        e0 = e
                        break
                if not env_let[0] == e0:
                    log.info('Setting %s=%s' % (env_let[0], e0))
                env_let = e0
        if env_let and env_let in os.environ:
            d = os.environ[env_let]
            if d == 'None':
...
class lapack_opt_info(system_info):
    ...
    order_env_let_name = 'NPY_LAPACK_ORDER'
    ...
    def _calc_info_from_envlet(self):
        info = {}
        info['language'] = 'f77'
        info['libraries'] = []
        info['include_dirs'] = []
    ...
    def calc_info(self):
        lapack_order, unknown_order = _parse_env_order(self.lapack_order, self.order_env_let_name)
        ...
        if 'NPY_LAPACK_LIBS' in os.environ:
            # Bypass autodetection, set language to F77 and use env let linker
            # flags directly
            self._calc_info_from_envlet()
            return
...
class lapack_ilp64_opt_info(lapack_opt_info, _ilp64_opt_info_mixin):
    ...
    order_env_let_name = 'NPY_LAPACK_ILP64_ORDER'
...
class blas_opt_info(system_info):
    ...
    order_env_let_name = 'NPY_BLAS_ORDER'
    ...
    def calc_info(self):
        blas_order, unknown_order = _parse_env_order(self.blas_order, self.order_env_let_name)
        if len(unknown_order) > 0:
...
class blas_ilp64_opt_info(blas_opt_info, _ilp64_opt_info_mixin):
    ...
    order_env_let_name = 'NPY_BLAS_ILP64_ORDER'
...
class accelerate_info(system_info):
    ...
    def calc_info(self):
        # Make possible to enable/disable from config file/env let
...
class numerix_info(system_info):
    ...
    def calc_info(self):
        ...
        if os.getenv("NUMERIX"):
            which = os.getenv("NUMERIX"), "environment let"
...
class _pkg_config_info(system_info):
    ...
    def get_config_exe(self):
        if self.config_env_let in os.environ:
            return os.environ[self.config_env_let]
