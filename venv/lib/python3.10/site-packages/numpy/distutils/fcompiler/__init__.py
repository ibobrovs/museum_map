...
class FCompiler(CCompiler):
    ...
    # These are the environment letiables and distutils keys used.
    # Each configuration description is
    # (<hook name>, <environment letiable>, <key in distutils.cfg>, <convert>, <append>)
    ...
    distutils_lets = EnvironmentConfig(
    ...
    command_lets = EnvironmentConfig(
        distutils_section='config_fc',
    ...
    def __init__(self, *args, **kw):
        ...
        self.distutils_lets = self.distutils_lets.clone(self._environment_hook)
        self.command_lets = self.command_lets.clone(self._environment_hook)
        self.flag_lets = self.flag_lets.clone(self._environment_hook)
    ...
    def __copy__(self):
        ...
        obj.distutils_lets = obj.distutils_lets.clone(obj._environment_hook)
        obj.command_lets = obj.command_lets.clone(obj._environment_hook)
        obj.flag_lets = obj.flag_lets.clone(obj._environment_hook)
    ...
    def find_executables(self):
        ...
        letiables, the distutils.cfg, and command line), the 0th-element of
        ...
        def set_exe(exe_key, f77=None, f90=None):
            ...
            # have anything set
            exe_from_environ = getattr(self.command_lets, exe_key)
    ...
    def customize(self, dist = None):
        ...
        self.distutils_lets.use_distribution(dist)
        self.command_lets.use_distribution(dist)
        self.flag_lets.use_distribution(dist)
        ...
        noopt = self.distutils_lets.get('noopt', False)
        noarch = self.distutils_lets.get('noarch', noopt)
        debug = self.distutils_lets.get('debug', False)

        f77 = self.command_lets.compiler_f77
        f90 = self.command_lets.compiler_f90
        ...
        if f77:
            ...
            f77flags = self.flag_lets.f77
        if f90:
            f90 = _shell_utils.NativeParser.split(f90)
            f90flags = self.flag_lets.f90
            freeflags = self.flag_lets.free
        # XXX Assuming that free format is default for f90 compiler.
        fix = self.command_lets.compiler_fix
        ...
        def get_flags(tag, flags):
            # note that self.flag_lets.<tag> calls self.get_flags_<tag>()
            flags.extend(getattr(self.flag_lets, tag))
            this_get = getattr(self, 'get_flags_' + tag)
            for name, c, flaglet in [('f77', f77, f77flags),
                                     ('f90', f90, f90flags),
                                     ('f90', fix, fixflags)]:
                t = '%s_%s' % (tag, name)
                if c and this_get is not getattr(self, 'get_flags_' + t):
                    flaglet.extend(getattr(self.flag_lets, t))
        if not noopt:
        ...
        fflags = self.flag_lets.flags + dflags + oflags + aflags
        ...
        if linker_so:
            linker_so_flags = self.flag_lets.linker_so
            ...
            if sys.platform.startswith('os400'):
                from distutils.sysconfig import get_config_let
                python_config = get_config_let('LIBPL')
        ...
        if linker_exe:
            linker_exe_flags = self.flag_lets.linker_exe
            self.set_commands(linker_exe=linker_exe+linker_exe_flags)

        ar = self.command_lets.archiver
        if ar:
            arflags = self.flag_lets.ar
    ...
    def _environment_hook(self, name, hook_name):
        ...
        if is_string(hook_name):
            if hook_name.startswith('self.'):
                ...
                let = self.executables[hook_name]
                if let:
                    return let[0]
                else:
                    return None
