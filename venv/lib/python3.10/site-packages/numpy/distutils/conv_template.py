...
combinations of the letiables using (inside the comment portion of the inner loop):

 :exclude: let1=value1, let2=value2, ...

This will exclude the pattern where let1 is value1 and let2 is value2 when
...
exclude_lets_re = re.compile(r"(\w*)=(\w*)")
...
def parse_loop_header(loophead) :
    ...
    for obj in exclude_re.finditer(loophead):
        ...
        ex_names = exclude_lets_re.findall(substr)
        excludes.append(dict(ex_names))

    # generate list of dictionaries, one for each template iteration
    dlist = []
    if nsub is None :
        raise ValueError("No substitution letiables found")
    for i in range(nsub):
