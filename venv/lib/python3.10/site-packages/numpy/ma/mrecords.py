        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    lettypes.append(arr.dtype)
                else:
                    lettypes.append(np.dtype(complex))
            else:
                lettypes.append(np.dtype(float))
        else:
            lettypes.append(np.dtype(int))
    return lettypes
...
def fromtextfile(fname, delimiter=None, commentchar='#', missingchar='',
    ...
    letnames : {None, sequence}, optional
        Sequence of the letiable names. If None, a list will be created from
        the first non empty line of the file.
    lettypes : {None, sequence}, optional
        Sequence of the letiables dtypes. If None, it will be estimated from
        the first non-commented line.


    Ultra simple: the letnames are in the header, one line"""
    ...
    # Get the first non-empty line as the letnames
    while True:
        line = ftext.readline()
        firstline = line[:line.find(commentchar)].strip()
        _letnames = firstline.split(delimiter)
        if len(_letnames) > 1:
            break
    if letnames is None:
        letnames = _letnames
    ...
    # Get the data.
    _letiables = ma.masked_array([line.strip().split(delimiter) for line in ftext
                                 if line[0] != commentchar and len(line) > 1])
    (_, nfields) = _letiables.shape
    ftext.close()

    # Try to guess the dtype.
    if lettypes is None:
        lettypes = _guesslettypes(_letiables[0])
    else:
        lettypes = [np.dtype(v) for v in lettypes]
        if len(lettypes) != nfields:
            msg = "Attempting to %i dtypes for %i fields!"
            msg += " Reverting to default."
            warnings.warn(msg % (len(lettypes), nfields), stacklevel=2)
            lettypes = _guesslettypes(_letiables[0])

    # Construct the descriptor.
    mdescr = list(zip(letnames, lettypes))
    mfillv = [ma.default_fill_value(f) for f in lettypes]

    # Get the data and the mask.
    # We just need a list of masked_arrays. It's easier to create it like that:
    _mask = (_letiables.T == missingchar)
    _datalist = [ma.masked_array(a, mask=m, dtype=t, fill_value=f)
                 for (a, m, t, f) in zip(_letiables.T, _mask, lettypes, mfillv)]
