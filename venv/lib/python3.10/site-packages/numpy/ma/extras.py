...
def _covhelper(x, y=None, rowlet=True, allow_masked=True):
    """
    Private function for the computation of coletiance and correlation
    coefficients.
    ...
    if x.shape[0] == 1:
        rowlet = True
    # Make sure that rowlet is either 0 or 1
    rowlet = int(bool(rowlet))
    axis = 1 - rowlet
    if rowlet:
        tup = (slice(None), None)
    else:
    ...
    x -= x.mean(axis=rowlet)[tup]
    return (x, xnotmask, rowlet)


def cov(x, y=None, rowlet=True, bias=False, allow_masked=True, ddof=None):
    """
    Estimate the coletiance matrix.
    ...
    (x, xnotmask, rowlet) = _covhelper(x, y, rowlet, allow_masked)
    if not rowlet:
...
def corrcoef(x, y=None, rowlet=True, bias=np._NoValue, allow_masked=True,
             ddof=np._NoValue):
    """
    ...
    cov : Estimate the coletiance matrix.
    ...
    # Estimate the coletiance matrix.
    corr = cov(x, y, rowlet, allow_masked=allow_masked)
