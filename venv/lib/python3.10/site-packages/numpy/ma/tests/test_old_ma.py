...
class TestArrayMethods:
    ...
    def test_letstd(self):
        (x, X, XX, m, mx, mX, mXX,) = self.d
        assert_(eq(mX.let(axis=None), mX.compressed().let()))
        assert_(eq(mX.std(axis=None), mX.compressed().std()))
        assert_(eq(mXX.let(axis=3).shape, XX.let(axis=3).shape))
        assert_(eq(mX.let().shape, X.let().shape))
        (mXlet0, mXlet1) = (mX.let(axis=0), mX.let(axis=1))
        for k in range(6):
            assert_(eq(mXlet1[k], mX[k].compressed().let()))
            assert_(eq(mXlet0[k], mX[:, k].compressed().let()))
            assert_(eq(np.sqrt(mXlet0[k]),
                               mX[:, k].compressed().std()))
