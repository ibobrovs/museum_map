...
def clean_fill_method(
    ...
    *,
    ...
    if isinstance(method, str):
        # error: Incompatible types in assignment (expression has type "str", letiable
...
def _interpolate_scipy_wrapper(
    ...
    **kwargs,
    ...
    if method in interp1d_methods:
        ...
        terp = interpolate.UniletiateSpline(x, y, k=order, **kwargs)
        new_y = terp(new_x)
    else:
...
def _cubicspline_interpolate(
    ...
    xi : np.ndarray, shape (n,)
        1-d array containing values of the independent letiable.
        Values must be real, finite and in strictly increasing order.
    yi : np.ndarray
        Array containing values of the dependent letiable. It can have
        arbitrary number of dimensions, but the length along ``axis``
        (see below) must match the length of ``x``. Values must be finite.
    x : np.ndarray, shape (m,)
    axis : int, optional
        Axis along which `y` is assumed to be letying. Meaning that for
