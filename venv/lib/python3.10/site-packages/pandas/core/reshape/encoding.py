...
def get_dummies(
    ...
    """
    Convert categorical letiable into dummy/indicator letiables.

    Each letiable is converted in as many 0/1 letiables as there are different
    values. Columns in the output are each named after a value; if the input is
    a DataFrame, the name of the original letiable is prepended to the value.
...
def from_dummies(
    ...
    """
    Create a categorical ``DataFrame`` from a ``DataFrame`` of dummy letiables.
    ...
    ----------
    data : DataFrame
        Data which contains dummy-coded letiables in form of integer columns of
        1's and 0's.
    ...
    default_category : None, Hashable or dict of Hashables, default None
        ...
        zero. Can be a single value for all letiables or a dict directly mapping
        the default categories to a prefix of a letiable.
    ...
    :class:`~pandas.Categorical` : Represent a categorical letiable in classic.
    ...
    letiables_slice = defaultdict(list)
    if sep is None:
        letiables_slice[""] = list(data.columns)
        ...
        for col in data_to_decode.columns:
            ...
            letiables_slice[prefix].append(col)
    else:
        raise TypeError(
    ...
    cat_data = {}
    for prefix, prefix_slice in letiables_slice.items():
        if sep is None:
