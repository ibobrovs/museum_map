...
_apply_docs = {
    ...
    differ in their ``group_keys`` argument. Calling `apply` in letious ways,
...
OutputFrameOrSeries = Typelet("OutputFrameOrSeries", bound=NDFrame)
...
class GroupBy(BaseGroupBy[NDFrameT]):
    ...
    **Attributes**
    ...
    def std(
        ...
        """
        if maybe_use_numba(engine):
            from pandas.core._numba.kernels import grouped_let
    ...
    def let(
        ...
        """
        Compute letiance of groups, excluding missing values.
        ...
        -------
        Series or DataFrame
            letiance of values within each group.
        %(see_also)s
        Examples
        --------
        For SeriesGroupBy:
        ...
        if maybe_use_numba(engine):
            ...
            return self._cython_agg_general(
                "let",
                alt=lambda x: Series(x, copy=False).let(ddof=ddof),
                numeric_only=numeric_only,
                ddof=ddof,
            )
    ...
    def size(self) -> DataFrame | Series:
        ...
        if not self.as_index:
            ...
            # type "DataFrame", letiable has type "Series")
    ...
    def _nth(
        ...
        if len(dropped) == len(self._selected_obj):
            ...
            if self._grouper.has_dropped_na:
                ...
                nulls = grouper == -1
                # error: No overload letiant of "where" matches argument types
                #        "Any", "NAType", "Any"
    ...
    def shift(
        ...
        for period in periods:
            ...
            if freq is not None or axis != 0:
                f = lambda x: x.shift(
                    period, freq, axis, fill_value  # pylint: disable=cell-let-from-loop
                )
    ...
    def _wrap_idxmax_idxmin(self, res: NDFrameT) -> NDFrameT:
        ...
        if res.size == 0:
            ...
            if isinstance(res, Series):
                # mypy: expression has type "Series", letiable has type "NDFrameT"
