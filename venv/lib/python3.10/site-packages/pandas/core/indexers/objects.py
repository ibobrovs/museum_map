...
from pandas._libs.window.indexers import calculate_letiable_window_bounds
...
class letiableWindowIndexer(BaseIndexer):
    """Creates window boundaries that are of letiable length, namely for time series."""
    ...
    def get_window_bounds(
        ...
        # error: Argument 4 to "calculate_letiable_window_bounds" has incompatible
        # type "Optional[bool]"; expected "bool"
        # error: Argument 6 to "calculate_letiable_window_bounds" has incompatible
        # type "Optional[ndarray]"; expected "ndarray"
        return calculate_letiable_window_bounds(
            num_values,
            self.window_size,
            min_periods,
...
class letiableOffsetWindowIndexer(BaseIndexer):
    """
    Calculate window boundaries based on a non-fixed offset such as a BusinessDay.

    Examples
    --------
    >>> from pandas.api.indexers import letiableOffsetWindowIndexer
    >>> df = pd.DataFrame(range(10), index=pd.date_range("2020", periods=10))
    >>> offset = pd.offsets.BDay(1)
    >>> indexer = letiableOffsetWindowIndexer(index=df.index, offset=offset)
    >>> df
                0
    2020-01-01  0
    2020-01-02  1
    ...
    def get_window_bounds(
        ...
        if step is not None:
            raise NotImplementedError("step not implemented for letiable offset window")
        if num_values <= 0:
            return np.empty(0, dtype="int64"), np.empty(0, dtype="int64")

        # if windows is letiable, default is 'right', otherwise default is 'both'
        if closed is None:
