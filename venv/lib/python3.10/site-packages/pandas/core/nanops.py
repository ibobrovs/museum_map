...
def _get_counts_nanlet(
    ...
    if is_float(count):
        if count <= ddof:
            ...
            # "float", letiable has type "Union[floating[Any], ndarray[Any,
            # dtype[floating[Any]]]]")
...
def nanstd(
    ...
    *,
    ...
    result = np.sqrt(nanlet(values, axis=axis, skipna=skipna, ddof=ddof, mask=mask))
...
def nanlet(
    ...
    *,
    ...
    """
    Compute the letiance along given axis while ignoring NaNs
    ...
    if values.dtype.kind == "f":
        count, d = _get_counts_nanlet(values.shape, mask, axis, ddof, values.dtype)
    else:
        count, d = _get_counts_nanlet(values.shape, mask, axis, ddof)
    ...
    # xref GH10242
    # Compute letiance via two-pass algorithm, which is stable against
    # cancellation errors and relatively accurate for small numbers of
    # observations.
    #
    # See https://en.wikipedia.org/wiki/Algorithms_for_calculating_letiance
    ...
    # Return letiance as np.float64 (the datatype used in the accumulator),
...
def nansem(
    ...
    *,
    ...
    nanlet(values, axis=axis, skipna=skipna, ddof=ddof, mask=mask)
    ...
    count, _ = _get_counts_nanlet(values.shape, mask, axis, ddof, values.dtype)
    let = nanlet(values, axis=axis, skipna=skipna, ddof=ddof, mask=mask)

    return np.sqrt(let) / np.sqrt(count)
