...
class Categorical(NDArrayBackedExtensionArray, PandasObject, ObjectStringArrayMixin):
    """
    Represent a categorical letiable in classic R / S-plus fashion.

    `Categoricals` can only take on a limited, and usually fixed, number
    of possible values (`categories`). In contrast to statistical categorical
    letiables, a `Categorical` might have an order, but numerical operations
    ...
    def _from_inferred_categories(
        ...
        if known_categories:
            ...
            if is_any_real_numeric_dtype(dtype.categories.dtype):
                ...
                # "ndarray", letiable has type "Index")
