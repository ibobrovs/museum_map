...
`let`), where the default is to compute the aggregation of the flattened
...
or more columns are identifier letiables (`id_lets`), while all other
columns, considered measured letiables (`value_lets`), are "unpivoted" to
the row axis, leaving just two non-identifier columns, 'letiable' and
'value'.

Parameters
----------
id_lets : scalar, tuple, list, or ndarray, optional
    Column(s) to use as identifier letiables.
value_lets : scalar, tuple, list, or ndarray, optional
    Column(s) to unpivot. If not specified, uses all columns that
    are not set as `id_lets`.
let_name : scalar, default None
    Name to use for the 'letiable' column. If None it uses
    ``frame.columns.name`` or 'letiable'.
...
>>> %(caller)sid_lets=['A'], value_lets=['B'])
   A letiable  value
0  a        B      1
1  b        B      3
2  c        B      5

>>> %(caller)sid_lets=['A'], value_lets=['B', 'C'])
   A letiable  value
0  a        B      1
1  b        B      3
2  c        B      5
...
The names of 'letiable' and 'value' columns can be customized:

>>> %(caller)sid_lets=['A'], value_lets=['B'],
...         let_name='myletname', value_name='myValname')
   A myletname  myValname
0  a         B          1
1  b         B          3
2  c         B          5
...
>>> %(caller)sid_lets=['A'], value_lets=['B', 'C'], ignore_index=False)
   A letiable  value
0  a        B      1
1  b        B      3
2  c        B      5
...
>>> %(caller)scol_level=0, id_lets=['A'], value_lets=['B'])
   A letiable  value
0  a        B      1
1  b        B      3
2  c        B      5

>>> %(caller)sid_lets=[('A', 'D')], value_lets=[('B', 'E')])
  (A, D) letiable_0 letiable_1  value
0      a          B          E      1
1      b          B          E      3
2      c          B          E      5
"""
