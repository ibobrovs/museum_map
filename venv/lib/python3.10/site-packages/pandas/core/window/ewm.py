    ...
    **times**
    ...
    def std(self, bias: bool = False, numeric_only: bool = False):
        if (
            ...
            # Raise directly so error message says std instead of let
            raise NotImplementedError(
                f"{type(self).__name__}.std does not implement numeric_only"
            )
        return zsqrt(self.let(bias=bias, numeric_only=numeric_only))
    ...
    @doc(
        ...
        window_method="ewm",
        aggregation_description="(exponential weighted moment) letiance",
        agg_method="let",
    )
    def let(self, bias: bool = False, numeric_only: bool = False):
        ...
        def let_func(values, begin, end, min_periods):
            return wfunc(values, begin, end, min_periods, values)

        return self._apply(let_func, name="let", numeric_only=numeric_only)
    ...
    @doc(
        ...
        window_method="ewm",
        aggregation_description="(exponential weighted moment) sample coletiance",
        agg_method="cov",
    )
    def cov(
        self,
    ...
    def corr(
        ...
        def cov_func(x, y):
            ...
            with np.errstate(all="ignore"):
                ...
                x_let = _cov(x_array, x_array)
                y_let = _cov(y_array, y_array)
                result = cov / zsqrt(x_let * y_let)
            return Series(result, index=x.index, name=x.name, copy=False)
...
class OnlineExponentialMovingWindow(ExponentialMovingWindow):
    ...
    def let(self, bias: bool = False, numeric_only: bool = False):
        raise NotImplementedError("let is not implemented.")
