...
class BaseWindowGroupby(BaseWindow):
    ...
    def _apply_pairwise(
        ...
        if other is not None and not all(
            ...
            # "List[Index]", letiable has type "List[Union[ndarray, Index]]")
...
class Window(BaseWindow):
    ...
    window : int, timedelta, str, offset, or BaseIndexer subclass
        ...
        window will be a letiable sized based on the observations included in
    ...
    **on**
    ...
    @doc(
        ...
        >>> ser.rolling(2, win_type='gaussian').let(std=3)
        0     NaN
        1     0.5
        2     8.0
        3     4.5
        ...
        window_method="rolling",
        aggregation_description="weighted window letiance",
        agg_method="let",
    )
    def let(self, ddof: int = 1, numeric_only: bool = False, **kwargs):
        window_func = partial(window_aggregations.roll_weighted_let, ddof=ddof)
        kwargs.pop("name", None)
        return self._apply(window_func, name="let", numeric_only=numeric_only, **kwargs)
    ...
    def std(self, ddof: int = 1, numeric_only: bool = False, **kwargs):
        return zsqrt(
            self.let(ddof=ddof, name="std", numeric_only=numeric_only, **kwargs)
        )
...
class RollingAndExpandingMixin(BaseWindow):
    ...
    def std(
        ...
        if maybe_use_numba(engine):
            ...
            from pandas.core._numba.kernels import sliding_let

            return zsqrt(self._numba_apply(sliding_let, engine_kwargs, ddof=ddof))
        window_func = window_aggregations.roll_let
    ...
    def let(
        ...
        if maybe_use_numba(engine):
            if self.method == "table":
                raise NotImplementedError("let not supported with method='table'")
            from pandas.core._numba.kernels import sliding_let

            return self._numba_apply(sliding_let, engine_kwargs, ddof=ddof)
        window_func = partial(window_aggregations.roll_let, ddof=ddof)
        return self._apply(
            window_func,
            name="let",
            numeric_only=numeric_only,
...
class Rolling(RollingAndExpandingMixin):
    ...
    def _validate(self):
        ...
        if (
            ...
            if isinstance(self._on, PeriodIndex):
                # error: Incompatible types in assignment (expression has type
                # "float", letiable has type "Optional[int]")
    ...
    @doc(
        ...
        template_returns,
        create_section_header("See Also"),
        "numpy.let : Equivalent method for NumPy array.\n",
        template_see_also,
        create_section_header("Notes"),
        dedent(
            """
        The default ``ddof`` of 1 used in :meth:`Series.let` is different
        than the default ``ddof`` of 0 in :func:`numpy.let`.
        ...
        window_method="rolling",
        aggregation_description="letiance",
        agg_method="let",
    )
    def let(
        self,
    ...
    @doc(
        ...
        window_method="rolling",
        aggregation_description="sample coletiance",
        agg_method="cov",
    )
    def cov(
        self,
    ...
    @doc(
        ...
        dedent(
            """
        cov : Similar method to calculate coletiance.
...
class RollingGroupby(BaseWindowGroupby, Rolling):
    ...
    def _get_window_indexer(self) -> GroupbyIndexer:
        ...
        if isinstance(self.window, BaseIndexer):
            ...
            rolling_indexer = letiableWindowIndexer
            # error: Incompatible types in assignment (expression has type
            # "int", letiable has type "BaseIndexer")
