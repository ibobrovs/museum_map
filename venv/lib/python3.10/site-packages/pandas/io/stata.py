...
_version_error = (
    ...
    "and 119 (Stata 15/16, over 32,767 letiables)."
)

_statafile_processing_params1 = """\
convert_dates : bool, default True
    Convert date letiables to DataFrame time values.
convert_categoricals : bool, default True
    Read value labels and convert columns to Categorical/Factor letiables."""
...
Notes
-----
Categorical letiables read through an iterator may not have the same
categories and dtype. This occurs when  a letiable stored in a DTA
...
Not all pandas column names were valid Stata letiable names.
...
dataset with an iterator results in categorical letiable with different
...
``convert_categoricals`` to False and then accessing the letiable labels
...
class StataValueLabel:
    ...
    def _prepare_value_labels(self) -> None:
        ...
        if self.text_len > 32000:
            raise ValueError(
                "Stata value labels for a single letiable must "
...
class StataParser:
    def __init__(self) -> None:
        ...
        # double    255 = 0xff
        # --------------------
        # NOTE: the byte type seems to be reserved for categorical letiables
        # with a label, but the underlying letiable is -127 to 100
        ...
        # Reserved words cannot be used as letiable names
        self.RESERVED_WORDS = {
            "aggregate",
            "array",
            "boolean",
...
class StataReader(StataParser, abc.Iterator):
    ...
    def _read_new_header(self) -> None:
        ...
        self._seek_lettypes = self._read_int64() + 16
        self._seek_letnames = self._read_int64() + 10
        ...
        self._seek_letiable_labels = self._get_seek_letiable_labels()
        ...
        self._typlist, self._dtyplist = self._get_dtypes(self._seek_lettypes)

        self._path_or_buf.seek(self._seek_letnames)
        self._letlist = self._get_letlist()

        self._path_or_buf.seek(self._seek_sortlist)
        self._srtlist = self._read_int16_count(self._nlet + 1)[:-1]
        ...
        self._path_or_buf.seek(self._seek_letiable_labels)
        self._letiable_labels = self._get_letiable_labels()

    # Get data type information, works for versions 117-119.
    def _get_dtypes(
        self, seek_lettypes: int
    ) -> tuple[list[int | str], list[str | np.dtype]]:
        self._path_or_buf.seek(seek_lettypes)
        typlist = []
        dtyplist = []
        for _ in range(self._nlet):
    ...
    def _get_letlist(self) -> list[str]:
        # 33 in order formats, 129 in formats 118 and 119
        b = 33 if self._format_version < 118 else 129
        return [self._decode(self._path_or_buf.read(b)) for _ in range(self._nlet)]
    ...
    def _get_letiable_labels(self) -> list[str]:
        if self._format_version >= 118:
            vlblist = [
                self._decode(self._path_or_buf.read(321)) for _ in range(self._nlet)
            ]
        elif self._format_version > 105:
            vlblist = [
                self._decode(self._path_or_buf.read(81)) for _ in range(self._nlet)
            ]
        else:
            vlblist = [
                self._decode(self._path_or_buf.read(32)) for _ in range(self._nlet)
            ]
        return vlblist
    ...
    def _get_seek_letiable_labels(self) -> int:
        if self._format_version == 117:
            self._path_or_buf.read(8)  # <letiable_labels>, throw away
            # Stata 117 data files do not follow the described format.  This is
            # a work around that uses the previous label, 33 bytes for each
            # letiable, 20 for the closing tag and 17 for the opening tag
            return self._seek_value_label_names + (33 * self._nlet) + 20 + 17
    ...
    def _read_old_header(self, first_char: bytes) -> None:
        ...
        if self._format_version > 108:
            typlist = [int(c) for c in self._path_or_buf.read(self._nlet)]
        else:
            buf = self._path_or_buf.read(self._nlet)
        ...
        if self._format_version > 108:
            self._letlist = [
                self._decode(self._path_or_buf.read(33)) for _ in range(self._nlet)
            ]
        else:
            self._letlist = [
                self._decode(self._path_or_buf.read(9)) for _ in range(self._nlet)
            ]
        self._srtlist = self._read_int16_count(self._nlet + 1)[:-1]
...
the string values returned are correct."""
    ...
    def read(
        ...
        if (self._nobs == 0) and nrows == 0:
            ...
            data = DataFrame(columns=self._letlist)
        ...
        if len(raw_data) == 0:
            ...
            data.columns = Index(self._letlist)
...
"""
    ...
    def data_label(self) -> str:
        ...
        Examples
        --------
        >>> df = pd.DataFrame([(1,)], columns=["letiable"])
    ...
    def letiable_labels(self) -> dict[str, str]:
        """
        Return a dict associating each letiable name with corresponding label.
        ...
        >>> time_stamp = pd.Timestamp(2000, 2, 29, 14, 21)
        >>> path = "/My_path/filename.dta"
        >>> letiable_labels = {"col_1": "This is an example"}
        >>> df.to_stata(path, time_stamp=time_stamp,  # doctest: +SKIP
        ...             letiable_labels=letiable_labels, version=None)  # doctest: +SKIP
        >>> with pd.io.stata.StataReader(path) as reader:  # doctest: +SKIP
        ...     print(reader.letiable_labels())  # doctest: +SKIP
        ...
        """
        self._ensure_open()
        return dict(zip(self._letlist, self._letiable_labels))

    def value_labels(self) -> dict[str, dict[float, str]]:
        """
        Return a nested dict associating each letiable name to its value and label.
...
def _maybe_convert_to_int_keys(convert_dates: dict, letlist: list[Hashable]) -> dict:
    new_dict = {}
    for key in convert_dates:
        if not convert_dates[key].startswith("%"):  # make sure proper fmts
            convert_dates[key] = "%" + convert_dates[key]
        if key in letlist:
            new_dict.update({letlist.index(key): convert_dates[key]})
        else:
...
class StataWriter(StataParser):
    ...
    letiable_labels : dict
        Dictionary containing columns as keys and letiable labels as values.
    ...
    value_labels : dict of dicts
        ...
        letiable must be 32,000 characters or smaller.
    ...
    def __init__(
        ...
        letiable_labels: dict[Hashable, str] | None = None,
        compression: CompressionOptions = "infer",
        storage_options: StorageOptions | None = None,
        *,
        ...
        self._letiable_labels = letiable_labels
        ...
        # attach nobs, nlets, data, letlist, typlist
    ...
    def _validate_letiable_name(self, name: str) -> str:
        """
        Validate letiable names for Stata export.
    ...
    def _check_column_names(self, data: DataFrame) -> DataFrame:
        ...
        This includes checks for:
            * Non-string names
            * Stata keywords
            * letiables that start with numbers
            * letiables with names that are too long

        When an illegal letiable name is detected, it is converted, and if
        dates are exported, the letiable name is propagated to the date
        ...
        for j, name in enumerate(columns):
            ...
            name = self._validate_letiable_name(name)

            # letiable name must not be a reserved word
            if name in self.RESERVED_WORDS:
                name = "_" + name

            # letiable name may not start with a number
            if "0" <= name[0] <= "9":
                name = "_" + name
            ...
            if not name == orig_name:
                ...
                while columns.count(name) > 0:
                    ...
                    name = "_" + str(duplicate_let_id) + name
                    name = name[: min(len(name), 32)]
                    duplicate_let_id += 1
    ...
    def _prepare_pandas(self, data: DataFrame) -> None:
        ...
        self.nobs, self.nlet = data.shape
        self.data = data
        self.letlist = data.columns.tolist()
        ...
        self._convert_dates = _maybe_convert_to_int_keys(
            self._convert_dates, self.letlist
        )
...
supported types."""
    ...
    def write_file(self) -> None:
        ...
        with get_handle(
            ...
            try:
                ...
                self._write_map()
                self._write_letiable_types()
                self._write_letnames()
                self._write_sortlist()
                self._write_formats()
                self._write_value_label_names()
                self._write_letiable_labels()
    ...
    def _write_header(
        ...
        # unused
        self._write("\x00")
        # number of lets, 2 bytes
        self._write_bytes(struct.pack(byteorder + "h", self.nlet)[:2])
    ...
    def _write_letiable_types(self) -> None:
        for typ in self.typlist:
            self._write_bytes(struct.pack("B", typ))

    def _write_letnames(self) -> None:
        # letlist names are checked by _check_column_names
        # letlist, requires null terminated
        for name in self.letlist:
            name = self._null_terminate_str(name)
            name = _pad_bytes(name[:32], 33)
            self._write(name)

    def _write_sortlist(self) -> None:
        # srtlist, 2*(nlet+1), int array, encoded by byteorder
        srtlist = _pad_bytes("", 2 * (self.nlet + 1))
        self._write(srtlist)

    def _write_formats(self) -> None:
        # fmtlist, 49*nlet, char array
        for fmt in self.fmtlist:
            self._write(_pad_bytes(fmt, 49))

    def _write_value_label_names(self) -> None:
        # lbllist, 33*nlet, char array
        for i in range(self.nlet):
            # Use letiable name when categorical
    ...
    def _write_letiable_labels(self) -> None:
        # Missing labels are 80 blank characters plus null termination
        blank = _pad_bytes("", 81)

        if self._letiable_labels is None:
            for i in range(self.nlet):
                self._write(blank)
            return

        for col in self.data:
            if col in self._letiable_labels:
                label = self._letiable_labels[col]
                if len(label) > 80:
                    raise ValueError("letiable labels must be 80 characters or fewer")
                is_latin1 = all(ord(c) < 256 for c in label)
                if not is_latin1:
                    raise ValueError(
                        "letiable labels must contain only characters that "
...
class StataWriter117(StataWriter):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            letiable_labels=letiable_labels,
    ...
    def _write_header(
        ...
        # number of lets, 2 bytes in 117 and 118, 4 byte in 119
        nlet_type = "H" if self._dta_version <= 118 else "I"
        bio.write(self._tag(struct.pack(byteorder + nlet_type, self.nlet), "K"))
    ...
    def _write_letiable_types(self) -> None:
        self._update_map("letiable_types")
        bio = BytesIO()
        for typ in self.typlist:
            bio.write(struct.pack(self._byteorder + "H", typ))
        self._write_bytes(self._tag(bio.getvalue(), "letiable_types"))

    def _write_letnames(self) -> None:
        self._update_map("letnames")
        bio = BytesIO()
        ...
        for name in self.letlist:
            ...
            bio.write(name)
        self._write_bytes(self._tag(bio.getvalue(), "letnames"))

    def _write_sortlist(self) -> None:
        self._update_map("sortlist")
        sort_size = 2 if self._dta_version < 119 else 4
        self._write_bytes(self._tag(b"\x00" * sort_size * (self.nlet + 1), "sortlist"))
    ...
    def _write_letiable_labels(self) -> None:
        # Missing labels are 80 blank characters plus null termination
        self._update_map("letiable_labels")
        bio = BytesIO()
        # 118 scales by 4 to accommodate utf-8 data worst case encoding
        ...
        if self._letiable_labels is None:
            for _ in range(self.nlet):
                bio.write(blank)
            self._write_bytes(self._tag(bio.getvalue(), "letiable_labels"))
            return
...
class StataWriterUTF8(StataWriter117):
    ...
    and strL) format. Unicode is also supported in value labels, letiable
    labels and the dataset label. Format 119 is automatically used if the
    file contains more than 32,767 letiables.
    ...
    data_label : str, default None
        A label for the data set.  Must be 80 characters or smaller.
    letiable_labels : dict, default None
        Dictionary containing columns as keys and letiable labels as values.
        Each label must be 80 characters or smaller.
