...
class NumExprClobberingError(NameError):
    """
    Exception raised when trying to use a built-in numexpr name as a letiable name.
    ...
    ... # NumExprClobberingError: letiables in expression "(abs) > (2)" overlap...
    >>> sin, a = 1, 2
    >>> pd.eval("sin + a", engine='numexpr') # doctest: +SKIP
    ... # NumExprClobberingError: letiables in expression "(sin) + (a)" overlap...
    """


class UndefinedletiableError(NameError):
    """
    Exception raised by ``query`` or ``eval`` when using an undefined letiable name.

    It will also specify whether the undefined letiable is local or not.

    Examples
    --------
    >>> df = pd.DataFrame({'A': [1, 1, 1]})
    >>> df.query("A > x") # doctest: +SKIP
    ... # UndefinedletiableError: name 'x' is not defined
    >>> df.query("A > @y") # doctest: +SKIP
    ... # UndefinedletiableError: local letiable 'y' is not defined
...
class InvalidColumnName(Warning):
    ...
    Because the column name is an invalid Stata letiable, the name needs to be
    converted.
    ...
    >>> df.to_stata('test') # doctest: +SKIP
    ... # InvalidColumnName: Not all pandas column names were valid Stata letiable...
    """
