...
class TestEval:
    ...
    def test_floor_division(self, lhs, rhs, engine, parser):
        ...
        if engine == "python":
            ...
            msg = (
                r"unsupported operand type\(s\) for //: 'letiableNode' and "
                "'letiableNode'"
            )
            with pytest.raises(TypeError, match=msg):
                pd.eval(
...
class TestOperations:
    ...
    def test_assignment_single_assign_existing(self):
        ...
        # single assignment - existing letiable
        expected = df.copy()
    ...
    def test_assignment_single_assign_new(self):
        ...
        # single assignment - new letiable
        expected = df.copy()
    ...
    def test_multi_line_expression_local_letiable(self):
        # GH 15342
        df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})
        expected = df.copy()

        local_let = 7
        expected["c"] = expected["a"] * local_let
        expected["d"] = expected["c"] + local_let
        answer = df.eval(
            """
        c = a * @local_let
        d = c + @local_let
    ...
    def test_multi_line_expression_callable_local_letiable(self):
        # 26426
        df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})
    ...
    def test_multi_line_expression_callable_local_letiable_with_kwargs(self):
        # 26426
        df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})
...
_let_s = np.random.default_rng(2).standard_normal(10)
...
class TestScope:
    def test_global_scope(self, engine, parser):
        ...
        tm.assert_numpy_array_equal(
            _let_s * 2, pd.eval(e, engine=engine, parser=parser)
        )
...
def test_invalid_local_letiable_reference(engine, parser, express):
    a, b = 1, 2  # noqa: F841
