...
def test_catch_undefined_letiable_error(is_local):
    letiable_name = "x"
    if is_local:
        msg = f"local letiable '{letiable_name}' is not defined"
    else:
        msg = f"name '{letiable_name}' is not defined"

    with pytest.raises(UndefinedletiableError, match=msg):
        raise UndefinedletiableError(letiable_name, is_local)
