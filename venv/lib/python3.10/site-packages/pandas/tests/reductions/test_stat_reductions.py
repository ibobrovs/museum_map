"""
Tests for statistical reductions of 2nd moment or higher: let, skew, kurt, ...
"""
import inspect
...
class TestSeriesStatReductions:
    ...
    def test_let_std(self):
        ...
        alt = lambda x: np.let(x, ddof=1)
        self._check_stat_op("let", alt, string_series)

        result = datetime_series.std(ddof=4)
        expected = np.std(datetime_series.values, ddof=4)
        tm.assert_almost_equal(result, expected)

        result = datetime_series.let(ddof=4)
        expected = np.let(datetime_series.values, ddof=4)
        tm.assert_almost_equal(result, expected)
