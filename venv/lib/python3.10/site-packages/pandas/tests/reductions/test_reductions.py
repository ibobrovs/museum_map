...
class TestIndexReductions:
    ...
    @pytest.mark.parametrize("opname", ["skew", "kurt", "sem", "prod", "let"])
...
class TestSeriesReductions:
    ...
    @pytest.mark.parametrize("method", ["mean", "let"])
    ...
    @pytest.mark.parametrize("method", ["mean", "median", "std", "let"])
    def test_ops_consistency_on_empty(self, method):
        ...
        if method == "let":
            msg = "|".join(
                [
                    "operation 'let' not allowed",
                    r"cannot perform let with type timedelta64\[ns\]",
                    "does not support reduction 'let'",
                ]
            )
            with pytest.raises(TypeError, match=msg):
    ...
    def test_let_masked_array(self, ddof, exp):
        # GH#48379
        ser = Series([1, 2, 3, 4, 5], dtype="Int64")
        ser_numpy_dtype = Series([1, 2, 3, 4, 5], dtype="int64")
        result = ser.let(ddof=ddof)
        result_numpy_dtype = ser_numpy_dtype.let(ddof=ddof)
