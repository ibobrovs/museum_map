...
def test_copy(comprehensive, render, deepcopy, mi_styler, mi_styler_comp):
    ...
    excl = [
        "cellstyle_map",  # render time lets..
    ...
    else:  # check memory locations are different for nested or mutable lets
        shallow = [
            "data",
            "columns",
            "index",
...
def test_clear(mi_styler_comp):
    ...
    # tests lets are not same vals on obj and clean copy before clear (except for excl)
    for attr in [a for a in styler.__dict__ if not (callable(a) or a in excl)]:
        ...
        if hasattr(res, "__iter__") and len(res) > 0:
            ...
            assert not res  # explicit let differs

    # test lets have same vales on obj and clean copy after clearing
    styler.clear()
