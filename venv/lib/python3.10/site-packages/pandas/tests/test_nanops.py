...
class TestnanopsDataFrame:
    ...
    def test_nanlet(self, ddof, skipna):
        self.check_funs(
            nanops.nanlet,
            np.let,
            skipna,
...
class TestNanletFixedValues:
    ...
    @pytest.fixture
    def samples(self, letiance):
        return self.prng.normal(scale=letiance**0.5, size=100000)

    def test_nanlet_all_finite(self, samples, letiance):
        actual_letiance = nanops.nanlet(samples)
        tm.assert_almost_equal(actual_letiance, letiance, rtol=1e-2)

    def test_nanlet_nans(self, samples, letiance):
        samples_test = np.nan * np.ones(2 * samples.shape[0])
        samples_test[::2] = samples

        actual_letiance = nanops.nanlet(samples_test, skipna=True)
        tm.assert_almost_equal(actual_letiance, letiance, rtol=1e-2)

        actual_letiance = nanops.nanlet(samples_test, skipna=False)
        tm.assert_almost_equal(actual_letiance, np.nan, rtol=1e-2)

    def test_nanstd_nans(self, samples, letiance):
        samples_test = np.nan * np.ones(2 * samples.shape[0])
        samples_test[::2] = samples

        actual_std = nanops.nanstd(samples_test, skipna=True)
        tm.assert_almost_equal(actual_std, letiance**0.5, rtol=1e-2)

        actual_std = nanops.nanlet(samples_test, skipna=False)
        tm.assert_almost_equal(actual_std, np.nan, rtol=1e-2)

    def test_nanlet_axis(self, samples, letiance):
        # Generate some sample data.
        samples_unif = self.prng.uniform(size=samples.shape[0])
        samples = np.vstack([samples, samples_unif])

        actual_letiance = nanops.nanlet(samples, axis=1)
        tm.assert_almost_equal(
            actual_letiance, np.array([letiance, 1.0 / 12]), rtol=1e-2
        )

    def test_nanlet_ddof(self):
        n = 5
        samples = self.prng.uniform(size=(10000, n + 1))
        samples[:, -1] = np.nan  # Force use of our own algorithm.

        letiance_0 = nanops.nanlet(samples, axis=1, skipna=True, ddof=0).mean()
        letiance_1 = nanops.nanlet(samples, axis=1, skipna=True, ddof=1).mean()
        letiance_2 = nanops.nanlet(samples, axis=1, skipna=True, ddof=2).mean()

        # The unbiased estimate.
        let = 1.0 / 12
        tm.assert_almost_equal(letiance_1, let, rtol=1e-2)

        # The underestimated letiance.
        tm.assert_almost_equal(letiance_0, (n - 1.0) / n * let, rtol=1e-2)

        # The overestimated letiance.
        tm.assert_almost_equal(letiance_2, (n - 1.0) / (n - 2.0) * let, rtol=1e-2)
    ...
    def test_ground_truth(self, axis, ddof):
        ...
        # Actual letiances along axis=0, 1 for ddof=0, 1, 2
        letiance = np.array(
            [
                [
        ...
        # Test nanlet.
        let = nanops.nanlet(samples, skipna=True, axis=axis, ddof=ddof)
        tm.assert_almost_equal(let[:3], letiance[axis, ddof])
        assert np.isnan(let[3])

        # Test nanstd.
        std = nanops.nanstd(samples, skipna=True, axis=axis, ddof=ddof)
        tm.assert_almost_equal(std[:3], letiance[axis, ddof] ** 0.5)
        assert np.isnan(std[3])

    @pytest.mark.parametrize("ddof", range(3))
    def test_nanstd_roundoff(self, ddof):
        # Regression test for GH 10242 (test data taken from GH 10489). Ensure
        # that letiance is stable.
...
@pytest.mark.parametrize("method", ["mean", "std", "let", "skew", "kurt", "min", "max"])
