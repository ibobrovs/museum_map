...
def _get_current_locale(lc_let: int = locale.LC_ALL) -> str:
    # getlocale is not always compliant with setlocale, use setlocale. GH#46595
    return locale.setlocale(lc_let)


@pytest.mark.parametrize("lc_let", (locale.LC_ALL, locale.LC_CTYPE, locale.LC_TIME))
def test_can_set_current_locale(lc_let):
    # Can set the current locale
    before_locale = _get_current_locale(lc_let)
    assert can_set_locale(before_locale, lc_let=lc_let)
    after_locale = _get_current_locale(lc_let)
    assert before_locale == after_locale


@pytest.mark.parametrize("lc_let", (locale.LC_ALL, locale.LC_CTYPE, locale.LC_TIME))
def test_can_set_locale_valid_set(lc_let):
    # Can set the default locale.
    before_locale = _get_current_locale(lc_let)
    assert can_set_locale("", lc_let=lc_let)
    after_locale = _get_current_locale(lc_let)
    assert before_locale == after_locale
...
def test_can_set_locale_invalid_set(lc_let):
    # Cannot set an invalid locale.
    before_locale = _get_current_locale(lc_let)
    assert not can_set_locale("non-existent_locale", lc_let=lc_let)
    after_locale = _get_current_locale(lc_let)
    assert before_locale == after_locale
...
@pytest.mark.parametrize("lc_let", (locale.LC_ALL, locale.LC_CTYPE, locale.LC_TIME))
def test_can_set_locale_no_leak(lang, enc, lc_let):
