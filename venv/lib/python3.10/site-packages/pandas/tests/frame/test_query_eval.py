...
class TestDataFrameQueryNumExprPandas:
    ...
    def test_query_scope(self, engine, parser):
        ...
        # no local letiable c
        with pytest.raises(
            UndefinedletiableError, match="local letiable 'c' is not defined"
        ):
            df.query("@a > b > @c", engine=engine, parser=parser)

        # no column named 'c'
        with pytest.raises(UndefinedletiableError, match="name 'c' is not defined"):
            df.query("@a > b > c", engine=engine, parser=parser)
    ...
    def test_query_doesnt_pickup_local(self, engine, parser):
        ...
        with pytest.raises(UndefinedletiableError, match="name 'sin' is not defined"):
    ...
    def test_nested_raises_on_local_self_reference(self, engine, parser):
        ...
        with pytest.raises(UndefinedletiableError, match="name 'df' is not defined"):
    ...
    def test_local_letiable_with_in(self, engine, parser):
        skip_if_no_pandas_parser(parser)
...
class TestDataFrameQueryNumExprPython(TestDataFrameQueryNumExprPandas):
    ...
    def test_query_numexpr_with_min_and_max_columns(self):
        ...
        regex_to_match = (
            r"letiables in expression \"\(min\) == \(1\)\" "
        ...
        regex_to_match = (
            r"letiables in expression \"\(max\) == \(1\)\" "
...
class TestDataFrameQueryStrings:
    ...
    def test_query_string_scalar_letiable(self, parser, engine):
        skip_if_no_pandas_parser(parser)
        df = DataFrame(
            {
...
class TestDataFrameQueryBacktickQuoting:
    ...
    def test_single_backtick_letiable_query(self, df):
        res = df.query("1 < `B B`")
        expect = df[1 < df["B B"]]
        tm.assert_frame_equal(res, expect)

    def test_two_backtick_letiables_query(self, df):
        res = df.query("1 < `B B` and 4 < `C C`")
        expect = df[(1 < df["B B"]) & (4 < df["C C"])]
        tm.assert_frame_equal(res, expect)

    def test_single_backtick_letiable_expr(self, df):
        res = df.eval("A + `B B`")
        expect = df["A"] + df["B B"]
        tm.assert_series_equal(res, expect)

    def test_two_backtick_letiables_expr(self, df):
        res = df.eval("`B B` + `C C`")
        expect = df["B B"] + df["C C"]
        tm.assert_series_equal(res, expect)

    def test_already_underscore_letiable(self, df):
        res = df.eval("`C_C` + A")
        expect = df["C_C"] + df["A"]
        tm.assert_series_equal(res, expect)
    ...
    def test_call_non_named_expression(self, df):
        """
        Only attributes and letiables ('named functions') can be called.
