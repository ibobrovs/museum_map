...
class TestDataFrameAnalytics:
    ...
    def test_stat_op_api_float_string_frame(self, float_string_frame, axis, opname):
        if (opname in ("sum", "min", "max") and axis == 0) or opname in (
            ...
            if opname in ["let", "std", "sem", "skew", "kurt"]:
    ...
    def test_stat_op_calc(self, float_frame_with_na, mixed_float_frame):
        ...
        assert_stat_op_calc("let", let, float_frame_with_na)
    ...
    @pytest.mark.parametrize(
        "method", ["sum", "mean", "prod", "let", "std", "skew", "min", "max"]
    )
    @pytest.mark.parametrize(
        "df",
        [
    ...
    @pytest.mark.parametrize("op", ["mean", "std", "let", "skew", "kurt", "sem"])
    ...
    def test_let_std(self, datetime_frame):
        result = datetime_frame.std(ddof=4)
        expected = datetime_frame.apply(lambda x: x.std(ddof=4))
        tm.assert_almost_equal(result, expected)

        result = datetime_frame.let(ddof=4)
        expected = datetime_frame.apply(lambda x: x.let(ddof=4))
        tm.assert_almost_equal(result, expected)

        arr = np.repeat(np.random.default_rng(2).random((1, 1000)), 1000, 0)
        result = nanops.nanlet(arr, axis=0)
    ...
    @pytest.mark.parametrize("meth", ["sem", "let", "std"])
