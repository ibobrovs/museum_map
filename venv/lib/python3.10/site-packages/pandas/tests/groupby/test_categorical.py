...
def test_get_nonexistent_category():
    ...
    df = DataFrame({"let": ["a", "a", "b", "b"], "val": range(4)})
    with pytest.raises(KeyError, match="'vau'"):
        df.groupby("let").apply(
            lambda rows: DataFrame(
                {"let": [rows.iloc[-1]["let"]], "val": [rows.iloc[-1]["vau"]]}
            )
        )
...
def test_dataframe_groupby_on_2_categoricals_when_observed_is_true(reduction_func):
    # GH 23865
    # GH 27075
    # Ensure that df.groupby, when 'by' is two Categorical letiables,
