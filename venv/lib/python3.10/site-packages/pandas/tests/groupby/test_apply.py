...
def test_apply_numeric_coercion_when_datetime():
    ...
    # columns.  letious GH issues were filed, the reproductions
    # for which are here.
...
def test_groupby_apply_all_none():
    ...
    # values. Issue 9684.
    test_df = DataFrame({"groups": [0, 0, 1, 1], "random_lets": [8, 7, 4, 5]})
...
def test_groupby_apply_none_first():
    ...
    test_df1 = DataFrame({"groups": [1, 1, 1, 2], "lets": [0, 1, 2, 3]})
    test_df2 = DataFrame({"groups": [1, 2, 2, 2], "lets": [0, 1, 2, 3]})
    ...
    expected1 = DataFrame({"groups": [1, 1], "lets": [0, 2]}, index=index1)
    expected2 = DataFrame({"groups": [2, 2], "lets": [1, 3]}, index=index2)
