...
def test_aggregate_str_func(tsframe, groupbyfunc):
    ...
    result = grouped.aggregate("let")
    expected = grouped.let()
    tm.assert_frame_equal(result, expected)

    # group frame by function dict
    result = grouped.agg({"A": "let", "B": "std", "C": "mean", "D": "sem"})
    expected = DataFrame(
        {
            "A": grouped["A"].let(),
            "B": grouped["B"].std(),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("let", [4.5] * 3, int, {"i": float, "j": float, "k": float}),
        ("sum", [5, 7, 9], "Int64", {"j": "int64"}),
        ("std", [4.5**0.5] * 3, "Int64", {"i": float, "j": float, "k": float}),
        ("let", [4.5] * 3, "Int64", {"i": "float64", "j": "float64", "k": "float64"}),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("let", [[2] * 2] * 3, {10: "float64", 20: "float64"}),
    ],
)
...
@pytest.mark.parametrize("func", ["min", "max", "let"])
