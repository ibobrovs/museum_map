...
@pytest.mark.parametrize(
    "agg_function",
    ["sum", "mean", "prod", "std", "let", "sem", "median"],
)
...
def test_omit_nuisance_agg(df, agg_function, numeric_only, using_infer_string):
    ...
    no_drop_nuisance = ("let", "std", "sem", "mean", "prod", "median")
...
@pytest.mark.parametrize("bug_let", [1, "a"])
def test_groupby_sum_on_nan_should_return_nan(bug_let):
    # GH 24196
    df = DataFrame({"A": [bug_let, bug_let, bug_let, np.nan]})
    if isinstance(bug_let, str):
        df = df.astype(object)
    dfgb = df.groupby(lambda x: x)
    result = dfgb.sum(min_count=1)

    expected_df = DataFrame(
        [bug_let, bug_let, bug_let, None], columns=["A"], dtype=df["A"].dtype
    )
    tm.assert_frame_equal(result, expected_df)
