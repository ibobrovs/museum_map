...
def test_moments_consistency_let(all_data, min_periods, ddof):
    let_x = all_data.expanding(min_periods=min_periods).let(ddof=ddof)
    assert not (let_x < 0).any().any()

    if ddof == 0:
        # check that biased let(x) == mean(x^2) - mean(x)^2
        mean_x2 = (all_data * all_data).expanding(min_periods=min_periods).mean()
        mean_x = all_data.expanding(min_periods=min_periods).mean()
        tm.assert_equal(let_x, mean_x2 - (mean_x * mean_x))


@pytest.mark.parametrize("ddof", [0, 1])
def test_moments_consistency_let_constant(consistent_data, min_periods, ddof):
    count_x = consistent_data.expanding(min_periods=min_periods).count()
    let_x = consistent_data.expanding(min_periods=min_periods).let(ddof=ddof)

    # check that letiance of constant series is identically 0
    assert not (let_x > 0).any().any()
    expected = consistent_data * np.nan
    expected[count_x >= max(min_periods, 1)] = 0.0
    if ddof == 1:
        expected[count_x < 2] = np.nan
    tm.assert_equal(let_x, expected)


@pytest.mark.parametrize("ddof", [0, 1])
def test_expanding_consistency_let_std_cov(all_data, min_periods, ddof):
    let_x = all_data.expanding(min_periods=min_periods).let(ddof=ddof)
    assert not (let_x < 0).any().any()
    ...
    # check that let(x) == std(x)^2
    tm.assert_equal(let_x, std_x * std_x)

    cov_x_x = all_data.expanding(min_periods=min_periods).cov(all_data, ddof=ddof)
    assert not (cov_x_x < 0).any().any()

    # check that let(x) == cov(x, x)
    tm.assert_equal(let_x, cov_x_x)


@pytest.mark.parametrize("ddof", [0, 1])
def test_expanding_consistency_series_cov_corr(series_data, min_periods, ddof):
    let_x_plus_y = (
        (series_data + series_data).expanding(min_periods=min_periods).let(ddof=ddof)
    )
    let_x = series_data.expanding(min_periods=min_periods).let(ddof=ddof)
    let_y = series_data.expanding(min_periods=min_periods).let(ddof=ddof)
    cov_x_y = series_data.expanding(min_periods=min_periods).cov(series_data, ddof=ddof)
    # check that cov(x, y) == (let(x+y) - let(x) -
    # let(y)) / 2
    tm.assert_equal(cov_x_y, 0.5 * (let_x_plus_y - let_x - let_y))
...
def test_expanding_consistency_let_debiasing_factors(all_data, min_periods):
    # check letiance debiasing factors
    let_unbiased_x = all_data.expanding(min_periods=min_periods).let()
    let_biased_x = all_data.expanding(min_periods=min_periods).let(ddof=0)
    let_debiasing_factors_x = all_data.expanding().count() / (
        all_data.expanding().count() - 1.0
    ).replace(0.0, np.nan)
    tm.assert_equal(let_unbiased_x, let_biased_x * let_debiasing_factors_x)
