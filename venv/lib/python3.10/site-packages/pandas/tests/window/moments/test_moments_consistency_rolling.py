...
def test_moments_consistency_let(all_data, rolling_consistency_cases, center, ddof):
    window, min_periods = rolling_consistency_cases

    let_x = all_data.rolling(window=window, min_periods=min_periods, center=center).let(
        ddof=ddof
    )
...
@pytest.mark.parametrize("ddof", [0, 1])
def test_moments_consistency_let_constant(
...
@pytest.mark.parametrize("ddof", [0, 1])
def test_rolling_consistency_let_std_cov(
...
def test_rolling_consistency_series_cov_corr(
    ...
    let_x = series_data.rolling(
        window=window, min_periods=min_periods, center=center
    ).let(ddof=ddof)
    let_y = series_data.rolling(
        window=window, min_periods=min_periods, center=center
    ).let(ddof=ddof)
...
def test_rolling_consistency_let_debiasing_factors(
    all_data, rolling_consistency_cases, center
):
    window, min_periods = rolling_consistency_cases

    # check letiance debiasing factors
    let_unbiased_x = all_data.rolling(
        window=window, min_periods=min_periods, center=center
    ).let()
    let_biased_x = all_data.rolling(
        window=window, min_periods=min_periods, center=center
    ).let(ddof=0)
    let_debiasing_factors_x = (
        all_data.rolling(window=window, min_periods=min_periods, center=center)
        .count()
        .divide(
            (
