...
from pandas.core.indexers.objects import letiableOffsetWindowIndexer
...
def test_letiable_window_nonunique(closed, expected, frame_or_series):
    # GH 20712
    index = DatetimeIndex(
        [
            "2011-01-01",
...
def test_letiable_offset_window_nonunique(closed, expected, frame_or_series):
    # GH 20712
    index = DatetimeIndex(
        [
            "2011-01-01",
    ...
    indexer = letiableOffsetWindowIndexer(index=index, offset=offset)
...
def test_missing_minp_zero_letiable():
    # https://github.com/pandas-dev/pandas/pull/18921
    x = Series(
        [np.nan] * 4,
...
@pytest.mark.parametrize(
    ...
    [
        ("let", 1, [5e33, 0, 0.5, 0.5, 2, 0]),
        ("std", 1, [7.071068e16, 0, 0.7071068, 0.7071068, 1.414214, 0]),
        ("let", 2, [5e33, 0.5, 0, 0.5, 2, 0]),
        ("std", 2, [7.071068e16, 0.7071068, 0, 0.7071068, 1.414214, 0]),
    ],
)
def test_rolling_let_numerical_issues(func, third_value, values):
    # GH: 37051
    ...
    tm.assert_series_equal(result, expected)
    # GH 42064
    # new `roll_let` will output 0.0 correctly
...
@pytest.mark.parametrize("method", ["let", "sum", "mean", "skew", "kurt", "min", "max"])
...
def test_rolling_let_floating_artifact_precision():
    # GH 37051
    s = Series([7, 5, 5, 5])
    result = s.rolling(3).let()
...
def test_rolling_let_same_value_count_logic(values, window, min_periods, expected):
    # GH 42064.

    expected = Series(expected)
    sr = Series(values)

    # With new algo implemented, result will be set to .0 in rolling let
    # if sufficient amount of consecutively same values are found.
    result_let = sr.rolling(window, min_periods=min_periods).let()
    ...
    tm.assert_series_equal(result_let, expected)
    # 2. zeros should be exactly the same since the new algo takes effect here
    tm.assert_series_equal(expected == 0, result_let == 0)

    # std should also pass as it's just a sqrt of let
