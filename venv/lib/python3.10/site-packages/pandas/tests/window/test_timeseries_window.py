...
class TestRollingTS:
    ...
    def test_invalid_minp(self, minp, regular):
        ...
        msg = (
            r"local letiable 'minp' referenced before assignment|"
    ...
    def test_ragged_let(self, ragged):
        df = ragged
        result = df.rolling(window="1s", min_periods=1).let(ddof=0)
        expected = df.copy()
        expected["B"] = [0.0] * 5
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="1s", min_periods=1).let(ddof=1)
        expected = df.copy()
        expected["B"] = [np.nan] * 5
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="3s", min_periods=1).let(ddof=0)
        expected = df.copy()
        expected["B"] = [0.0] + [0.25] * 4
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="5s", min_periods=1).let(ddof=1)
        expected = df.copy()
