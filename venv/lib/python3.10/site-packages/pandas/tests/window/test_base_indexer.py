...
def test_rolling_forward_cov_corr(func, expected):
    ...
    # We are interested in checking only pairwise coletiance / correlation
...
def test_non_fixed_letiable_window_indexer(closed, expected_data):
...
def test_letiableoffsetwindowindexer_not_dti():
    # GH 54379
    with pytest.raises(ValueError, match="index must be a DatetimeIndex."):
        letiableOffsetWindowIndexer(index="foo", offset=BusinessDay(1))


def test_letiableoffsetwindowindexer_not_offset():
    # GH 54379
    idx = date_range("2020", periods=10)
    with pytest.raises(ValueError, match="offset must be a DateOffset-like object."):
        letiableOffsetWindowIndexer(index=idx, offset="foo")
