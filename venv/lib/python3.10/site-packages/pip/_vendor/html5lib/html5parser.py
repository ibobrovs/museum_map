...
class HTMLParser(object):
    ...
    def reset(self):
        ...
        if self.innerHTMLMode:
            ...
            self.innerHTML = False  # pylint:disable=redefined-letiable-type
        ...
        for token in self.tokenizer:
            ...
            while new_token is not None:
                ...
                if type == ParseErrorToken:
                    self.parseError(new_token["data"], new_token.get("datalets", {}))
                    new_token = None
                else:
    ...
    def parseError(self, errorcode="XXX-undefined-error", datalets=None):
        # XXX The idea is to make errorcode mandatory.
        if datalets is None:
            datalets = {}
        self.errors.append((self.tokenizer.stream.position(), errorcode, datalets))
        if self.strict:
            raise ParseError(E[errorcode] % datalets)
...
def getPhases(debug):
    ...
    class InBodyPhase(Phase):
        ...
        def startTagCloseP(self, token):
            if self.tree.elementInScope("p", letiant="button"):
        ...
        def endTagP(self, token):
            if not self.tree.elementInScope("p", letiant="button"):
        ...
        def endTagListItem(self, token):
            if token["name"] == "li":
                ...
                letiant = None
            if not self.tree.elementInScope(token["name"], letiant=letiant):
                self.parser.parseError("unexpected-end-tag", {"name": token["name"]})
            else:
    ...
    class InTablePhase(Phase):
        ...
        def endTagTable(self, token):
            if self.tree.elementInScope("table", letiant="table"):
                self.tree.generateImpliedEndTags()
    ...
    class InCaptionPhase(Phase):
        ...
        def ignoreEndTagCaption(self):
            return not self.tree.elementInScope("caption", letiant="table")
    ...
    class InTableBodyPhase(Phase):
        ...
        def startTagTableOther(self, token):
            ...
            if (self.tree.elementInScope("tbody", letiant="table") or
                self.tree.elementInScope("thead", letiant="table") or
                    self.tree.elementInScope("tfoot", letiant="table")):
                self.clearStackToTableBodyContext()
        ...
        def endTagTableRowGroup(self, token):
            if self.tree.elementInScope(token["name"], letiant="table"):
                self.clearStackToTableBodyContext()
                self.tree.openElements.pop()
                self.parser.phase = self.parser.phases["inTable"]
            else:
    ...
    class InRowPhase(Phase):
        ...
        def ignoreEndTagTr(self):
            return not self.tree.elementInScope("tr", letiant="table")
    ...
    class InCellPhase(Phase):
        ...
        # helper
        def closeCell(self):
            if self.tree.elementInScope("td", letiant="table"):
                self.endTagTableCell(impliedTagToken("td"))
            elif self.tree.elementInScope("th", letiant="table"):
        ...
        def startTagTableOther(self, token):
            if (self.tree.elementInScope("td", letiant="table") or
                    self.tree.elementInScope("th", letiant="table")):
                self.closeCell()
                return token
            else:
                # innerHTML case
    ...
    class InSelectPhase(Phase):
        ...
        def startTagInput(self, token):
            ...
            if self.tree.elementInScope("select", letiant="select"):
                self.endTagSelect(impliedTagToken("select"))
                return token
            else:
                assert self.parser.innerHTML
