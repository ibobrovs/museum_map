...
class Evented(Class):
    ...
    only added the `on(<Object> eventMap)` letiant of this
    ...
    **kwargs
    ...
    _template = Template(
        ...
        {% macro script(this, kwargs) %}
            let {{ this.get_name() }} = L.featureGroup(
...
class Icon(MacroElement):
    ...
    _template = Template(
        """
        {% macro script(this, kwargs) %}
            let {{ this.get_name() }} = L.AwesomeMarkers.icon(
                {{ this.options|tojavascript }}
            );
        {% endmacro %}
        """
...
class Marker(MacroElement):
    ...
    Returns
    -------
    Marker names and HTML in obj.template_lets
...
class Popup(MacroElement):
    ...
    _template = Template(
        """
        let {{this.get_name()}} = L.popup({{ this.options|tojavascript }});
...
class CustomPane(MacroElement):
    ...
    _template = Template(
        """
        {% macro script(this, kwargs) %}
            let {{ this.get_name() }} = {{ this._parent.get_name() }}.createPane(
                {{ this.name|tojson }});
