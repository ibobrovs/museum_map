...
let mapPane=this._map.getPanes().mapPane;let point=mapPane._leaflet_pos;this._el.style[HeatmapOverlay.CSS_TRANSFORM]='translate('+
-Math.round(point.x)+'px,'+
-Math.round(point.y)+'px)';this._update();},_update:function(){let bounds,zoom,scale;let generatedData={max:this._max,min:this._min,data:[]};bounds=this._map.getBounds();zoom=this._map.getZoom();scale=Math.pow(2,zoom);if(this._data.length==0){if(this._heatmap){this._heatmap.setData(generatedData);}
return;}
let latLngPoints=[];let radiusMultiplier=this.cfg.scaleRadius?scale:1;let localMax=0;let localMin=0;let valueField=this.cfg.valueField;let len=this._data.length;while(len--){let entry=this._data[len];let value=entry[valueField];let latlng=entry.latlng;if(!bounds.contains(latlng)){continue;}
...
generatedData.data=latLngPoints;this._heatmap.setData(generatedData);},setData:function(data){this._max=data.max||this._max;this._min=data.min||this._min;let latField=this.cfg.latField||'lat';let lngField=this.cfg.lngField||'lng';let valueField=this.cfg.valueField||'value';let data=data.data;let len=data.length;let d=[];while(len--){let entry=data[len];let latlng=new L.LatLng(entry[latField],entry[lngField]);let dataObj={latlng:latlng};dataObj[valueField]=entry[valueField];if(entry.radius){dataObj.radius=entry.radius;}
d.push(dataObj);}
...
this._draw();}});HeatmapOverlay.CSS_TRANSFORM=(function(){let div=document.createElement('div');let props=['transform','WebkitTransform','MozTransform','OTransform','msTransform'];for(let i=0;i<props.length;i++){let prop=props[i];if(div.style[prop]!==undefined){return prop;}}
