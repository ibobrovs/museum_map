    ...
    **kwargs:
    ...
    _template = Template(
        ...
        {% macro script(this, kwargs) %}
            let {{ this.get_name() }} = new L.RegularPolygonMarker(
    ...
    **kwargs
    ...
    _template = Template(
        ...
        function {{ this.get_name() }}_pointToLayer(feature, latlng) {
            let opts = {{ this.marker.options | tojavascript }};
        ...
        let {{ this.get_name() }} = L.geoJson(null, {
                ...
                for key, value in content.items():
                    if isinstance(value, MacroElement):
                        ...
                        if value._parent is None:
                            ...
                            value.render()
                        # Replace objects with their Javascript let names:
...
class TopoJson(JSCSSMixin, Layer):
    ...
    _template = Template(
        """
        {% macro script(this, kwargs) %}
            let {{ this.get_name() }}_data = {{ this.data|tojson }};
            let {{ this.get_name() }} = L.geoJson(
                topojson.feature(
                    {{ this.get_name() }}_data,
...
class Choropleth(FeatureGroup):
    ...
    key_on: string, default None
        letiable in the `geo_data` GeoJSON file to bind the data to. Must
    ...
    Returns
    -------
    GeoJSON data layer in obj.template_lets
...
class DivIcon(MacroElement):
    ...
    _template = Template(
        """
        {% macro script(this, kwargs) %}
            let {{ this.get_name() }} = L.divIcon({{ this.options|tojavascript }});
        {% endmacro %}
        """
    )  # noqa
...
class Control(JSCSSMixin, Class):
    ...
    _template = Template(
        """
      {% macro script(this, kwargs) %}
          let {{ this.get_name() }} = new L.Control.{{this._name}}(
