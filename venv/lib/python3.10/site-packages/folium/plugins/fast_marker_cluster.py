    ...
    **kwargs
    ...
    _template = Template(
        ...
        {% macro script(this, kwargs) %}
            let {{ this.get_name() }} = (function(){
                {{ this.callback }}

                let data = {{ this.data|tojson }};
                let cluster = L.markerClusterGroup({{ this.options|tojavascript }});
        ...
        if callback is None:
            self.callback = """
                let callback = function (row) {
                    ...
                    let marker = L.marker(new L.LatLng(row[0], row[1]));
                    marker.setIcon(icon);
                    return marker;
                };"""
        else:
            self.callback = f"let callback = {callback};"
